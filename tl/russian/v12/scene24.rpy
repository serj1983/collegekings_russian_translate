# TODO: Translation updated at 2021-11-10 00:46

# game/v12/scene24.rpy:19
translate russian v12_simplr_convo_c0497226:

    # u "(Oh shit, how the fuck did I get back here? Damn, that was crazy!)"
    u "(О чёрт, как, чёрт возьми, я сюда вернулся? Черт, это было безумие!)"

# game/v12/scene24.rpy:24
translate russian v12_simplr_convo_0fa55aff:

    # u "(I need to sleep this off, but I'm kind of wide awake right now. I'll just check out my phone.)"
    u "(Мне нужно выспаться, но сейчас я вроде как бодрствую. Я просто проверю свой телефон)."

# game/v12/scene24.rpy:30
translate russian v12_simplr_convo_aeeb154a:

    # u "(Randomly waking up in the middle of the night, with nothing to do except stare at the wall, yayyy.)"
    u "(Случайное пробуждение посреди ночи, когда нечем заняться, кроме как пялиться в стену, ураааа)."

# game/v12/scene24.rpy:35
translate russian v12_simplr_convo_9d27cce1:

    # u "(Guess I can check out my phone.)"
    u "(Думаю, я могу проверить свой телефон)."

# game/v12/scene24.rpy:40
translate russian v12_simplr_convo_2e66f1eb:

    # u "(Haven't checked out Simplr in a while.)"
    u "(Давно не заглядывал в Simplr)."

# game/v12/scene24.rpy:62
translate russian v12_simplr_convo_c0f47f91:

    # u "(Very interesting... *Chuckles* Now, I need to sleep.)"
    u "(Очень интересно... *Смеется* Теперь мне нужно поспать.)"

# game/v12/scene24.rpy:68
translate russian v12_simplr_convo_89080957:

    # u "(Well, no matches, I'm just gonna take my sorry ass to sleep.)"
    u "(Ну, никаких совпадений, я просто собираюсь уложить свою жалкую задницу спать.)"

