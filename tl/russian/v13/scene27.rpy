# TODO: Translation updated at 2021-11-10 00:46

# game/v13/scene27.rpy:19
translate russian v13s27_f39f4c96:

    # u "Wow... Perfect timing, who is it this time?"
    u "Вау... Как вовремя, кто это на этот раз?"

# game/v13/scene27.rpy:32
translate russian v13s27_PhoneContinueAmber_88d710ce:

    # u "(I should check my phone.)"
    u "(Я должен проверить свой телефон.)"

# game/v13/scene27.rpy:38
translate russian v13s27_PhoneContinueAmber_87d7e556:

    # u "(She wants me to drag my ass over there without knowing a damn thing about what we're doing?)"
    u "(Она хочет, чтобы я тащил туда свою задницу, ни черта не зная о том, что мы делаем?)"

# game/v13/scene27.rpy:43
translate russian v13s27_PhoneContinueAmber_f7e60c9a:

    # u "*Sighs* (I'd rather just go instead of getting my shit rocked by Amber.)"
    u "*Вздох* (Я бы предпочел просто уйти, вместо того чтобы получить от Эмбер.)"

# game/v13/scene27.rpy:48
translate russian v13s27_PhoneContinueAmber_c1ba7e32:

    # u "(I don't even know where Ryan and Imre are.)"
    u "(Я даже не знаю, где Райан и Имре)."

# game/v13/scene27.rpy:53
translate russian v13s27_PhoneContinueAmber_c74c29f0:

    # u "*Chuckles* (Oh well.)"
    u "*Хихикает* (О, хорошо.)"

