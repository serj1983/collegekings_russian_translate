# TODO: Translation updated at 2021-11-10 00:46

# game/v8/scene34.rpy:67
translate russian v8_tues_noon_29659097:

    # u "(I should check in with Julia.)"
    u "(Я должен поговорить с Джулией.)"

# game/v8/scene34.rpy:72
translate russian v8_tues_noon_29659097_1:

    # u "(I should check in with Julia.)"
    u "(Я должен поговорить с Джулией.)"

# game/v8/scene34.rpy:79
translate russian v8_tues_noon_29a406f8:

    # u "(I wonder how Chloe's doing. I should text her.)"
    u "(Интересно, как дела у Хлои. Я должен написать ей.)"

# game/v8/scene34.rpy:84
translate russian v8_tues_noon_29a406f8_1:

    # u "(I wonder how Chloe's doing. I should text her.)"
    u "(Интересно, как дела у Хлои. Я должен написать ей.)"

# game/v8/scene34.rpy:100
translate russian phn_chloe13_8861c6b3:

    # u "(I should talk to Chloe.)"
    u "(Мне нужно поговорить с Хлоей.)"

# game/v8/scene34.rpy:105
translate russian phn_chloe13_da3ee415:

    # u "(Guess I'm meeting Chloe.)"
    u "(Похоже, я встречусь с Хлоей.)"

# game/v8/scene34.rpy:109
translate russian phn_chloe13_29659097:

    # u "(I should check in with Julia.)"
    u "(Я должен поговорить с Джулией.)"

translate russian strings:

    # game/v8/scene34.rpy:10
    old "I bet you were"
    new "Держу пари, так и было"

    # game/v8/scene34.rpy:11
    old "I miss them"
    new "Я скучаю по ним"

    # game/v8/scene34.rpy:12
    old "I know... we should def try again"
    new "Я знаю... мы должны попробовать ещё раз"

    # game/v8/scene34.rpy:13
    old "I'm on my way!"
    new "Я уже в пути!"

    # game/v8/scene34.rpy:14
    old "Hahaha"
    new "Хахаха"

    # game/v8/scene34.rpy:15
    old "Did you finish studying?"
    new "Ты закончила учёбу?"

    # game/v8/scene34.rpy:18
    old "Nothing. Bored."
    new "Нет.Это скучно"

    # game/v8/scene34.rpy:22
    old "Good. Bored. You?"
    new "Хорошо. Скучно. А ты?"

    # game/v8/scene34.rpy:23
    old "Same!"
    new "Тоже самое!"

    # game/v8/scene34.rpy:27
    old "Can't say it's all done, but I sure am!"
    new "Не могу сказать, что всё готово, но я уверена!"

    # game/v8/scene34.rpy:29
    old "So when can I see you again?"
    new "Так когда мы сможем снова увидеться?"

    # game/v8/scene34.rpy:32
    old "I'm actually getting hungry. Wanna go grab a bite?"
    new "Я уже проголодался. Хочешь перекусить?"

    # game/v8/scene34.rpy:33
    old "I better get back to it. I have so much work to get done. Just wanted to check on you and see how you're doing."
    new "Я лучше вернусь к этому. У меня так много работы, которую нужно сделать. Просто хотел узнать, как у тебя дела."

    # game/v8/scene34.rpy:38
    old "Um... how's now? ;)"
    new "Эм... сейчас ;)"

    # game/v8/scene34.rpy:39
    old "Now's perfect! Should I cum to your place or you wanna cum here? ;)"
    new "Сейчас идеально! Мне кончить у тебя или ты хочешь кончить здесь ;)"

    # game/v8/scene34.rpy:40
    old "I was thinking more along the lines of food. I'm hungry!"
    new "Я больше думала о еде. Я проголодалась"

    # game/v8/scene34.rpy:41
    old "Well I got something to fill your mouth up"
    new "Ну, у меня есть кое-что, чтобы наполнить твой рот"

    # game/v8/scene34.rpy:42
    old "Maybe after real food :P"
    new "Может быть, после полноценной еды :P"

    # game/v8/scene34.rpy:43
    old "What are you in the mood for? Lady's choice"
    new "Какое у тебя желание!? Выбор леди"

    # game/v8/scene34.rpy:49
    old "As friends"
    new "Как друзья"

    # game/v8/scene34.rpy:50
    old "Ok, yeah. I'm actually starving!"
    new "Ладно, да. Я действительно умираю с голоду"

    # game/v8/scene34.rpy:54
    old "It was nice talking to you"
    new "Было приятно поговорить с тобой"

    # game/v8/scene34.rpy:55
    old "You too"
    new "И мне"

    # game/v8/scene34.rpy:58
    old "There's a new Japanese place right down the road"
    new "Прямо по дороге открылось новое японское заведение"

    # game/v8/scene34.rpy:59
    old "Sounds delicious. Meet you there!"
    new "Звучит восхитительно. Встретимся там"

    # game/v8/scene34.rpy:87
    old "Hey, sexy :* Haven't seen you in a while"
    new "Привет, секси :* Давно тебя не видел."

    # game/v8/scene34.rpy:88
    old "OMG! I was just thinking about you!"
    new "ОМГ! Я только что думала о тебе!"

    # game/v8/scene34.rpy:89
    old "I was thinking about you too"
    new "Я тоже думал о тебе"

    # game/v8/scene34.rpy:90
    old "Awww!"
    new "Оууу!"

    # game/v8/scene34.rpy:91
    old "I was thinking about your lips :*"
    new "Я думал о твоих губах :*"

    # game/v8/scene34.rpy:92
    old "Whatcha been up to?"
    new "Чем занимаешься?"

    # game/v8/scene34.rpy:94
    old "Hey, how you been?"
    new "Привет, как дела?"

