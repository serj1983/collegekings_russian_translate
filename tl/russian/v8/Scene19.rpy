# TODO: Translation updated at 2021-11-10 00:46

# game/v8/Scene19.rpy:119
translate russian v8s19_phoneCheck1_e6825ef5:

    # u "I should reply to Emily"
    u "Я должен ответить Эмили"

# game/v8/Scene19.rpy:128
translate russian v8s19_phoneCheck1_c478a397:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:134
translate russian v8s19_phoneCheck1_c478a397_1:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:187
translate russian v8s19_phoneCheck2_e6825ef5:

    # u "I should reply to Emily"
    u "Я должен ответить Эмили"

# game/v8/Scene19.rpy:196
translate russian v8s19_phoneCheck2_c478a397:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:202
translate russian v8s19_phoneCheck2_c478a397_1:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

translate russian strings:

    # game/v8/Scene19.rpy:11
    old "It's only fair, right? Make us even"
    new "Это же не справедливо, правда? Сделай нас равными"

    # game/v8/Scene19.rpy:21
    old "Wow, better than I thought"
    new "Вау, лучше, чем я думала"

    # game/v8/Scene19.rpy:22
    old "So you thought about it?"
    new "Так ты думала об этом?"

    # game/v8/Scene19.rpy:24
    old "Aw man you're driving me crazy"
    new "Ты сводишь меня с ума"

    # game/v8/Scene19.rpy:25
    old "So do something about it"
    new "Так что сделай что-нибудь с этим"

    # game/v8/Scene19.rpy:26
    old "Now? What about you?"
    new "Сейчас? А как насчёт тебя?"

    # game/v8/Scene19.rpy:34
    old "Aw fuck"
    new "Ох, вау"

    # game/v8/Scene19.rpy:35
    old "You like?"
    new "Тебе нравится?"

    # game/v8/Scene19.rpy:36
    old "I love! You're so hot!"
    new "Ещё как! Ты такая секси!"

    # game/v8/Scene19.rpy:37
    old "What would you do if you were here?"
    new "Что бы ты сделал, если бы был рядом?"

    # game/v8/Scene19.rpy:38
    old "I would eat you until you couldn't take it anymore"
    new "Я буду поедать тебя до тех пор, пока ты не сойдёшь с ума"

    # game/v8/Scene19.rpy:39
    old "And then what?"
    new "И что потом?"

    # game/v8/Scene19.rpy:40
    old "I'd fuck you so good you'd scream my name"
    new "Я бы трахнул тебя так хорошо, что ты бы выкрикнула моё имя"

    # game/v8/Scene19.rpy:41
    old "I'm close. Will you finish with me?"
    new "Я уже близко. Ты кончишь со мной?"

    # game/v8/Scene19.rpy:42
    old "Oh, God yes!"
    new "Ох, Да!"

    # game/v8/Scene19.rpy:43
    old "NOW!"
    new "СЕЙЧАС!"

    # game/v8/Scene19.rpy:46
    old "Holy shit, Amber! You're amazing!"
    new "Святые небеса, Эмбер! Ты потрясающая!"

    # game/v8/Scene19.rpy:47
    old "You weren't too bad yourself. Next time we need to do this in person"
    new "Ты и сам был не так уж плох. В следующий раз мы должны сделать это лично"

    # game/v8/Scene19.rpy:48
    old "Give me 5 minutes ;)"
    new "Дай мне 5 минут ;)"

    # game/v8/Scene19.rpy:49
    old "You're so cute! But it's time for bed. Dream about me ;)"
    new "Ты такой милый! Но пора спать. Помечтай обо мне ;)"

    # game/v8/Scene19.rpy:50
    old "With pleasure! Night!"
    new "С удовольствием! Спокойной ночи!"

    # game/v8/Scene19.rpy:53
    old "Good. Maybe someday we can think about things in the same room and see what happens"
    new "Хорошо. Может быть, когда-нибудь мы сможем думать о всяком в одной комнате и посмотрим, что произойдёт"

    # game/v8/Scene19.rpy:54
    old "Please do"
    new "Конечно"

    # game/v8/Scene19.rpy:55
    old "I guess I better get back to studying. I keep getting distracted"
    new "Думаю, мне лучше вернуться к учёбе. Я продолжаю отвлекаться"

    # game/v8/Scene19.rpy:56
    old "I'm not gonna be able to think of anything else now. I'm done studying"
    new "Теперь я не смогу думать ни о чём другом. Я закончил учиться"

    # game/v8/Scene19.rpy:57
    old "Well, sleep tight then ;)"
    new "Ну, тогда спи спокойно ;)"

    # game/v8/Scene19.rpy:60
    old "Great! See you there!"
    new "Отлично! Увидимся там!"

    # game/v8/Scene19.rpy:63
    old "You sure you're not mad?"
    new "Ты уверен, что не злишься?"

    # game/v8/Scene19.rpy:64
    old "No, not at all. Just beat. I'd love to go some other time"
    new "Нет, совсем нет. Просто устал. Я с удовольствием пойду в другой раз."

    # game/v8/Scene19.rpy:65
    old "Okay talk to you soon"
    new "Хорошо тогда на связи!"

    # game/v8/Scene19.rpy:66
    old "Goodnight"
    new "Спокойной ночи"

    # game/v8/Scene19.rpy:76
    old "Hey u up?"
    new "Привет, как дела?"

    # game/v8/Scene19.rpy:77
    old "Always for you ;)"
    new "Хорошо, не могу дождаться когда увижу тебя ;)"

    # game/v8/Scene19.rpy:78
    old "That's what I was hoping to hear"
    new "Это то, что я надеялась услышать"

    # game/v8/Scene19.rpy:79
    old "I can be even more up if you want..."
    new "Я могу быть ещё лучше, если захочешь"

    # game/v8/Scene19.rpy:80
    old "Really? Just like that?"
    new "Правда? Вот так просто?"

    # game/v8/Scene19.rpy:81
    old "What can I say? You're hot"
    new "Что я могу сказать? Ты горячая"

    # game/v8/Scene19.rpy:82
    old "You're pretty hot yourself and I'm... thinking about things"
    new "Ты и сам довольно горяч, а я... думаю о том"

    # game/v8/Scene19.rpy:83
    old "What kind of things? Same things I'm thinking? ;)"
    new "О чём именно? О том же, о чём думаю я? ;)"

    # game/v8/Scene19.rpy:84
    old "I think so"
    new "Думаю, да."

    # game/v8/Scene19.rpy:85
    old "Wanna see what thinking about you has done to me?"
    new "Хочешь увидеть, что мысли о тебе сделали со мной?"

    # game/v8/Scene19.rpy:86
    old "I look at your pic all the time...when I'm thinking about things"
    new "Я часто смотрю на твою фотографию... когда думаю о разном..."

    # game/v8/Scene19.rpy:90
    old "Hey, Sweetie, what are you up to?"
    new "Эй, милый, что делаешь?"

    # game/v8/Scene19.rpy:91
    old "Nothing, just catching up on some homework. You having a good night?"
    new "Ничего, просто доделываю домашнюю работу. Хорошо провела ночь?"

    # game/v8/Scene19.rpy:92
    old "It would be better if you were here..."
    new "Было бы лучше, если бы ты был здесь..."

    # game/v8/Scene19.rpy:94
    old "I could use some snuggles."
    new "Мне бы не помешали обнимашки!"

    # game/v8/Scene19.rpy:95
    old "Aww, I'd love to get some snuggles. When can I see you again?"
    new "О, я бы с удовольствием пообнимался. Когда я смогу увидеть тебя снова?"

    # game/v8/Scene19.rpy:96
    old "I have a big test coming up but after that? I miss you"
    new "У меня скоро большой тест, может после него? Я скучаю по тебе."

    # game/v8/Scene19.rpy:97
    old "I miss you too. It's a date. Just let me know"
    new "Я тоже скучаю по тебе. Это будет свидание. Просто дай мне знать."

    # game/v8/Scene19.rpy:106
    old "Hey, I was thinking"
    new "Эй, я тут подумала"

    # game/v8/Scene19.rpy:107
    old "Uh oh that can't be good ;)"
    new "О, это не может быть хорошо ;)"

    # game/v8/Scene19.rpy:108
    old "Wanna meet up at the arcade?"
    new "Хочешь сходить в игровой зал?"

    # game/v8/Scene19.rpy:111
    old "I feel so bad about homecoming and want to make it up to you. My treat!"
    new "Я так плохо себя чувствовала на выпускном вечере и хочу загладить свою вину. Я угощаю!"

    # game/v8/Scene19.rpy:112
    old "Sure! Sounds like fun. I can be there in a few minutes"
    new "Конечно! Звучит заманчиво. Я могу быть там через несколько минут"

    # game/v8/Scene19.rpy:113
    old "I would but it's getting late and I haven't even started Mr. Lee's project"
    new "Я бы сделал это, но уже поздно, а я даже не начал проект мистера Ли"

# TODO: Translation updated at 2023-06-21 15:57

# game/v8/Scene19.rpy:115
translate russian sun_eve_room_e6825ef5:

    # u "I should reply to Emily"
    u "Я должен ответить Эмили"

# game/v8/Scene19.rpy:123
translate russian sun_eve_room_c478a397:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:129
translate russian sun_eve_room_c478a397_1:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:185
translate russian sun_eve_room_e6825ef5_1:

    # u "I should reply to Emily"
    u "Я должен ответить Эмили"

# game/v8/Scene19.rpy:193
translate russian sun_eve_room_c478a397_2:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

# game/v8/Scene19.rpy:199
translate russian sun_eve_room_c478a397_3:

    # u "(I think I'll get an early night)"
    u "(Я думаю, мне нужно пораньше лечь спать.)"

