# TODO: Translation updated at 2021-11-10 00:46
# Перевёл: serj1983 (post.serj@gmail.com)

# game/v11/scene2.rpy:17
translate russian v11_nora_chloe_hallway_d8f50366:

    # u "Hey Nora, are there a lot of people going on the trip?"
    u "Эй, Нора, много людей собирается в поездку?"

# game/v11/scene2.rpy:24
translate russian v11_nora_chloe_hallway_0b51ee5c:

    # no "Yeah, there's plenty. I'm surprised we actually got enough people."
    no "Да, много. Я удивлена, что у нас собралось достаточно людей."

# game/v11/scene2.rpy:29
translate russian v11_nora_chloe_hallway_de4b684b:

    # no "Plenty. I'm surprised we actually got enough people."
    no "Много. Я удивлена, что мы собрали достаточно людей."

# game/v11/scene2.rpy:34
translate russian v11_nora_chloe_hallway_ae8ed2dd:

    # u "And yet you don't seem too psyched about it."
    u "И все же ты, кажется, не слишком взволнована этим."

# game/v11/scene2.rpy:39
translate russian v11_nora_chloe_hallway_9f7aba1d:

    # no "Well, Chris and I have been having... moments. So that could cause unneeded stress. Then there's Chloe."
    no "Ну, у нас с Крисом были... отношения. Так что это ненужный стресс. А тут еще Хлоя."

# game/v11/scene2.rpy:44
translate russian v11_nora_chloe_hallway_55351329:

    # u "Sounds like you're having a bit of a rough time?"
    u "Похоже, у тебя сейчас трудные времена?"

# game/v11/scene2.rpy:49
translate russian v11_nora_chloe_hallway_dd5dae7c:

    # no "Well, I asked Mr. Lee and Ms. Rose to put out a sign up sheet because I was afraid I wasn't going to get enough people."
    no "Ну, я попросила мистера Ли и мисс Роуз вывесить регистрационный лист, потому что я боялась, что у меня не будет достаточно людей."

# game/v11/scene2.rpy:51
translate russian v11_nora_chloe_hallway_b29dbedb:

    # no "It kinda got out of hand and with everything going on it's all just a bit stressful."
    no "Это вышло из-под контроля, и со всем происходящим это просто немного напрягает."

# game/v11/scene2.rpy:56
translate russian v11_nora_chloe_hallway_6b7e0d80:

    # u "I'm sure things with you and Chris will be better in Europe... I mean it's Europe after all."
    u "Я уверена, что в Европе у вас с Крисом все будет лучше... В конце концов, это же Европа."

# game/v11/scene2.rpy:61
translate russian v11_nora_chloe_hallway_109022f6:

    # no "You would think, but he said he's gonna be glued to his phone the entire time. The most I can hope for is a good conversation on the plane."
    no "Ага, но он сказал, что все это время будет приклеен к своему телефону. Самое большее, на что я могу надеяться, это на хороший разговор в самолете."

# game/v11/scene2.rpy:74
translate russian v11_nora_chloe_hallway_b165d970:

    # u "Well I'll be there, I hope that's something good. *Chuckles*"
    u "Что ж, я буду там, надеюсь, это будет что-то хорошее. *Хихикает*"

# game/v11/scene2.rpy:80
translate russian v11_nora_chloe_hallway_d68658e3:

    # no "*Chuckles* It's definitely something."
    no "* Хихикает* Надеюсь."

# game/v11/scene2.rpy:85
translate russian v11_nora_chloe_hallway_d6f28d06:

    # no "I just wish he'd make more of an effort."
    no "Я просто хочу, чтобы он приложил больше усилий."

# game/v11/scene2.rpy:93
translate russian v11_nora_chloe_hallway_3606c7a0:

    # u "Just try to enjoy yourself anyway."
    u "Постарайтесь получить удовольствие от поездки."

# game/v11/scene2.rpy:99
translate russian v11_nora_chloe_hallway_79b29d16:

    # no "Thanks for trying, but I think it's clear where his priorities are."
    no "Спасибо, но думаю, что ясно, где его приоритеты."

# game/v11/scene2.rpy:104
translate russian v11_nora_chloe_hallway_208ff044:

    # no "Yeah. Right."
    no "Да. Точно."

# game/v11/scene2.rpy:109
translate russian v11_nora_chloe_hallway_fe01b239:

    # u "What about Chloe? Think you two can make up?"
    u "А как же Хлоя? Думаешь, вы двое сможете помириться?"

# game/v11/scene2.rpy:114
translate russian v11_nora_chloe_hallway_e7c50e09:

    # no "I don't think being in another country is gonna make a difference."
    no "Не думаю, что пребывание в другой стране что-то изменит."

# game/v11/scene2.rpy:127
translate russian v11_nora_chloe_hallway_41cc61fa:

    # u "I really think you should try to work things out with her. I mean... you guys are sorority sisters, you're supposed to stick together..."
    u "Ты должна попытаться наладить с ней отношения. Я имею в виду... вы же сестры, вы должны держаться вместе..."

# game/v11/scene2.rpy:132
translate russian v11_nora_chloe_hallway_39fb3809:

    # no "And? That doesn't change that she's fake. I can't get along with fake people. What are you, her publicist?"
    no "И? Это не меняет того, что она фальшивая. Я не могу ладить с фальшивыми людьми. Ты что, ее публицист?"

# game/v11/scene2.rpy:137
translate russian v11_nora_chloe_hallway_9f623797:

    # u "No I just... nevermind."
    u "Нет, я просто... неважно."

# game/v11/scene2.rpy:145
translate russian v11_nora_chloe_hallway_72fa809b:

    # u "This trip was your thing, don't let her or anyone else mess that up."
    u "Эта поездка была твоей идеей, не позволяй ей или кому-то еще все испортить."

# game/v11/scene2.rpy:150
translate russian v11_nora_chloe_hallway_cb4a0aa6:

    # no "I'll try my best."
    no "Я буду стараться изо всех сил."

# game/v11/scene2.rpy:155
translate russian v11_nora_chloe_hallway_9b9ac36b:

    # no "Oh, hey Charli."
    no "О, привет, Чарли."

# game/v11/scene2.rpy:160
translate russian v11_nora_chloe_hallway_70f33159:

    # charli "Hey there."
    charli "Привет."

# game/v11/scene2.rpy:165
translate russian v11_nora_chloe_hallway_a24774b0:

    # no "Have you two had a chance to meet yet?"
    no "Вы уже успели познакомиться?"

# game/v11/scene2.rpy:170
translate russian v11_nora_chloe_hallway_a6cd06ac:

    # u "Umm, I don't think so."
    u "Я так не думаю."

# game/v11/scene2.rpy:175
translate russian v11_nora_chloe_hallway_5bc86ff9:

    # no "Well, while you guys do that, I'm gonna run."
    no "Ну, пока вы этим займитесь, а я побегу."

# game/v11/scene2.rpy:180
translate russian v11_nora_chloe_hallway_bee6b96a:

    # u "*Chuckles* Alright, catch you later."
    u "*Хихикает* Ладно, увидимся позже."

# game/v11/scene2.rpy:185
translate russian v11_nora_chloe_hallway_c491777c:

    # charli "Well, nice to meet you. Like Nora said, my name's Charli."
    charli "Что ж, приятно с тобой познакомиться. Как и сказала Нора, меня зовут Чарли."

# game/v11/scene2.rpy:190
translate russian v11_nora_chloe_hallway_9eefc16b:

    # u "I'm [name]. So you're a student here?"
    u "Я [name]. Ты тоже студент?"

# game/v11/scene2.rpy:195
translate russian v11_nora_chloe_hallway_e693042c:

    # charli "Yeah, an exchange student. I've been here since the start of the year, but I'm living off campus so I don't know that many students."
    charli "Да, студент по обмену. Я здесь с начала года, но живу за пределами кампуса, поэтому знаю не так много студентов."

# game/v11/scene2.rpy:196
translate russian v11_nora_chloe_hallway_097e9baf:

    # charli "Nora and I have talked a few times, but that's about it as I'm often tied up doing other things..."
    charli "Мы с Норой несколько раз разговаривали, но на этом все, так как я часто занят другими делами..."

# game/v11/scene2.rpy:203
translate russian v11_nora_chloe_hallway_59e06418:

    # charli "So, are you going on this Europe trip? Mr. Lee invited me to go. But I'm not sure I actually want to."
    charli "Итак, вы собираетесь в эту поездку по Европе? Мистер Ли пригласил меня поехать. Но я не уверен, что хочу ехать."

# game/v11/scene2.rpy:208
translate russian v11_nora_chloe_hallway_69bc0531:

    # u "Yeah, I'll be there. What are you unsure of? It's gonna be a great time and there's some really cool events planned. Plus, you can meet all the students you haven't met."
    u "Да, я поеду. А ты почему не хочешь? Это будет отличное время, запланировано несколько крутых мероприятий. Кроме того, ты можеть познакомиться со многими студентами, которых ты еще не встречал."

# game/v11/scene2.rpy:213
translate russian v11_nora_chloe_hallway_046a9ada:

    # charli "Well, I have to admit that's pretty convincing. *Chuckles* I guess I should start packing."
    charli "Ха, я должен признать, что это довольно убедительно. *Хихикает* Думаю, мне пора собираться."

# game/v11/scene2.rpy:218
translate russian v11_nora_chloe_hallway_d01485fb:

    # u "Haha, that's probably a good idea."
    u "Ха-ха, хорошая идея."

# game/v11/scene2.rpy:223
translate russian v11_nora_chloe_hallway_53fccf40:

    # charli "Are you close with anyone that's going on the trip?"
    charli "Ты близок с кем-нибудь из тех, кто едет в поездку?"

# game/v11/scene2.rpy:228
translate russian v11_nora_chloe_hallway_b61dc49a:

    # u "Yeah, a few people. I'm closer with some more than others though."
    u "Да, несколько человек. С некоторыми я сблизился больше, чем с другими."

# game/v11/scene2.rpy:233
translate russian v11_nora_chloe_hallway_2a8c72ea:

    # charli "*Chuckles* You said you're closer with some more than others? I can tell you're close with Nora, but who else?"
    charli "*Хихиканье* Ты сказал, что с некоторыми ты ближе, чем с другими? Я могу сказать, что ты близок с Норой, но с кем еще?"

# game/v11/scene2.rpy:234
translate russian v11_nora_chloe_hallway_5ba335a7:

    # charli "Anyone you'd recommend for me to start getting to know? Or even avoid?"
    charli "Кого бы ты посоветовал мне для начала знакомства? Кого избегать?"

# game/v11/scene2.rpy:239
translate russian v11_nora_chloe_hallway_cba00f62:

    # u "Depends on the company you like to keep..."
    u "Зависит от того, какую компанию ты предпочитаешь..."

# game/v11/scene2.rpy:240
translate russian v11_nora_chloe_hallway_a339922b:

    # u "If you're looking for a more mellow time then Lauren or Lindsey may be nice to talk to, but if you have a more wild side then getting to know Amber or Aubrey is probably right up your alley."
    u "Если ты ищешь более спокойное времяпрепровождения, то Лорен или Линдси могут быть приятными собеседницами, в противном случае, знакомство с Эмбер или Обри, вероятно, будет как раз по твоей части."

# game/v11/scene2.rpy:243
translate russian v11_nora_chloe_hallway_281fc0a8:

    # u "In terms of people to avoid, It's always best to avoid Grayson. *Laughs*"
    u "Что касается людей, которых стоит избегать, то лучше всего избегать Грейсона. *Смеется*"

# game/v11/scene2.rpy:245
translate russian v11_nora_chloe_hallway_2e1925ee:

    # u "In terms of people to avoid, It's always best to keep a good distance between yourself and Cameron. *Laughs*"
    u "Что касается людей, которых следует избегать, то всегда лучше держать хорошую дистанцию между собой и Кэмероном. *Смеется*"

# game/v11/scene2.rpy:250
translate russian v11_nora_chloe_hallway_4b6e6225:

    # charli "And what kind of company are you?"
    charli "А ты в какой компании относишься?"

# game/v11/scene2.rpy:258
translate russian v11_nora_chloe_hallway_d06e7fb9:

    # u "I mean, I like to have fun, but I wouldn't consider myself as wild as some of my friends are."
    u "Мне нравится веселиться, но я бы не считал себя таким диким, как некоторые из моих друзей."

# game/v11/scene2.rpy:264
translate russian v11_nora_chloe_hallway_fc9f702c:

    # u "Haha... I guess you could say I'm here for the full college experience."
    u "Хаха... Думаю, можно сказать, что я здесь, чтобы получить полный опыт обучения в колледже."

# game/v11/scene2.rpy:269
translate russian v11_nora_chloe_hallway_ebe90f5e:

    # charli "Nora mentioned that we'll have some time to do our own thing and explore. Anything you're looking forward to?"
    charli "Нора упомянула, что у нас будет немного времени, чтобы заняться своими делами и исследовать достопримечательности. Ты чего-нибудь ждешь от поездки?"

# game/v11/scene2.rpy:276
translate russian v11_nora_chloe_hallway_c1b3d7b3:

    # u "Well, we'll be in another country surrounded by beautiful women... I'm sure you can imagine what I plan on doing. *Chuckles* Really though, I just wanna have fun no matter what."
    u "Что ж, мы будем в другой стране, в окружении красивых женщин... Я уверен, ты можешь себе представить, что я планирую сделать. *Хихикает* На самом деле, я просто хочу повеселиться, несмотря ни на что."

# game/v11/scene2.rpy:281
translate russian v11_nora_chloe_hallway_4dcc0870:

    # charli "With that attitude there's no way this will be a boring trip! *Laughs*"
    charli "С таким отношением это ни за что не будет скучной поездкой! *Смеется*"

# game/v11/scene2.rpy:286
translate russian v11_nora_chloe_hallway_ab42054f:

    # u "I don't know... sleeping? *Chuckles*"
    u "Я не знаю... спать? * Хихикает*"

# game/v11/scene2.rpy:291
translate russian v11_nora_chloe_hallway_0850e11c:

    # charli "Oh, you're a fan of hotel mattresses? Haha."
    charli "О, ты поклонник гостиничных матрасов?"

# game/v11/scene2.rpy:296
translate russian v11_nora_chloe_hallway_fa2eeaab:

    # u "There's nothing better than going to sleep and waking up in a fresh hotel bed. *Chuckles*"
    u "Нет ничего лучше, чем заснуть и проснуться в свежей гостиничной постели. *Хихикает*"

# game/v11/scene2.rpy:301
translate russian v11_nora_chloe_hallway_65f1ce7d:

    # charli "That's something I can definitely agree with and they clean up after you."
    charli "Это то, с чем я точно могу согласиться, и они убирают за тобой."

# game/v11/scene2.rpy:306
translate russian v11_nora_chloe_hallway_3e251465:

    # u "Haha, exactly."
    u "Ха-ха, точно."

# game/v11/scene2.rpy:311
translate russian v11_nora_chloe_hallway_b9df75ac:

    # charli "Well, as much as I'm enjoying this, I guess I should go start packing for the trip... See you around?"
    charli "Что ж, как бы мне это ни нравилось, думаю, мне пора начинать собираться в поездку... Увидимся?"

# game/v11/scene2.rpy:316
translate russian v11_nora_chloe_hallway_22a94b01:

    # u "For sure!"
    u "Конечно!"

# game/v11/scene2.rpy:321
translate russian v11_nora_chloe_hallway_a3f4292d:

    # u "(Hmm, he must be off campus a lot. I'm pretty sure I've never seen him before. He seems pretty nice.)"
    u "(Хм, должно быть, он часто бывает за пределами кампуса. Я уверен, что никогда не видел его раньше. Он кажется довольно приятным.)"

# game/v11/scene2.rpy:341
translate russian v11_nora_chloe_hallway_b918daa7:

    # "(I need to get home and pack.)"
    "(Мне нужно вернуться домой и собрать вещи.)"

# game/v11/scene2.rpy:348
translate russian v11_nora_chloe_hallway_21a98069:

    # u "Hey Chloe."
    u "Привет, Хлоя."

# game/v11/scene2.rpy:353
translate russian v11_nora_chloe_hallway_a06225e9:

    # cl "Why do you keep trying to talk to me?"
    cl "Почему ты продолжаешь пытаться поговорить со мной?"

# game/v11/scene2.rpy:366
translate russian v11_nora_chloe_hallway_8f5d4c37:

    # u "I'm sorry, you know? Sorry for how things went. I know you're mad at me, but I really want us to move past this and at least be friends."
    u "Мне очень жаль? Извини за то, как все прошло. Я знаю, ты злишься на меня, но я хочу, чтобы мы оставили это в прошлом и, по крайней мере, были друзьями."

# game/v11/scene2.rpy:371
translate russian v11_nora_chloe_hallway_a1c74379:

    # cl "*Sighs* I'm not even really that mad anymore."
    cl "*Вздох* Я уже даже не так сильно злюсь."

# game/v11/scene2.rpy:376
translate russian v11_nora_chloe_hallway_eddec9da:

    # u "But you just..."
    u "Но ты..."

# game/v11/scene2.rpy:381
translate russian v11_nora_chloe_hallway_5b97337a:

    # cl "Sometimes I get a little carried away keeping up appearances."
    cl "Иногда я немного увлекаюсь соблюдением приличий."

# game/v11/scene2.rpy:386
translate russian v11_nora_chloe_hallway_b6061d29:

    # u "You were definitely doing a good job, I thought you were still pissed."
    u "Ты хорошо справилась, я думал, ты все еще злишься."

# game/v11/scene2.rpy:391
translate russian v11_nora_chloe_hallway_41e031bf:

    # cl "Yeah no, I'm sorry. You know, I have a few minutes before my next class. Wanna go outside and talk for a few minutes?"
    cl "Да нет, мне очень жаль. Знаешь, у меня есть несколько минут до следующего урока. Хочешь выйти на улицу и поговорить немного?"

# game/v11/scene2.rpy:404
translate russian v11_nora_chloe_hallway_37fe551f:

    # u "Sure."
    u "Конечно."

# game/v11/scene2.rpy:414
translate russian v11_nora_chloe_hallway_71d2b8be:

    # cl "Are you excited for the trip?"
    cl "Ты волнуешься перед поездкой?"

# game/v11/scene2.rpy:419
translate russian v11_nora_chloe_hallway_1b28a406:

    # u "Of course."
    u "Безусловно."

# game/v11/scene2.rpy:424
translate russian v11_nora_chloe_hallway_f9f95e51:

    # cl "I am too. I'm a little nervous knowing I'm going to have to be around some people that I don't really get along with, but I'll still have fun."
    cl "Я тоже. Я немного нервничаю, зная, что мне придется быть рядом с некоторыми людьми, с которыми я не лажу, но мне все равно будет весело."

# game/v11/scene2.rpy:429
translate russian v11_nora_chloe_hallway_ac82a12e:

    # u "Yeah, no reason not to enjoy yourself..."
    u "Да, нет причин не получать удовольствие..."

# game/v11/scene2.rpy:434
translate russian v11_nora_chloe_hallway_23edfa13:

    # cl "I wonder what will be the crazy thing that happens."
    cl "Интересно, что же такого безумного произойдет?"

# game/v11/scene2.rpy:439
translate russian v11_nora_chloe_hallway_09332657:

    # u "What do you mean?"
    u "Что ты имеешь в виду?"

# game/v11/scene2.rpy:444
translate russian v11_nora_chloe_hallway_f7bcefcc:

    # cl "You know how every time you go on a trip something crazy always happens that everyone talks about? I went on a class trip a couple years ago and one of our classmates had a baby."
    cl "Ты знаешь, что каждый раз, когда ты отправляешься в путешествие, всегда происходит что-то сумасшедшее, о чем все говорят? Пару лет назад я ездила в классную поездку, и у одной из наших одноклассниц родился ребенок."

# game/v11/scene2.rpy:446
translate russian v11_nora_chloe_hallway_e1794fd5:

    # cl "*Laughs* She didn't even know she was pregnant."
    cl "*Смеется* Она даже не знала, что беременна."

# game/v11/scene2.rpy:451
translate russian v11_nora_chloe_hallway_7d966cb0:

    # u "*Chuckles* That's horrible."
    u "*Хихикает* Это ужасно."

# game/v11/scene2.rpy:456
translate russian v11_nora_chloe_hallway_6dbb700d:

    # cl "Yeah, it was pretty bad at the time. I guess we'll see what people will talk about after Europe..."
    cl "Да, в то время все было очень плохо. Думаю, мы посмотрим, о чем люди будут говорить после Европы..."

# game/v11/scene2.rpy:461
translate russian v11_nora_chloe_hallway_bc7666e9:

    # u "Maybe you'll trip over the Queen's gown while we're in London... That'd be funny now and a few years later."
    u "Может быть, ты споткнешься о платье королевы, когда мы будем в Лондоне... Это было бы забавно сейчас и спустя несколько лет."

# game/v11/scene2.rpy:466
translate russian v11_nora_chloe_hallway_c24522e3:

    # cl "If it isn't caught on camera it never happened."
    cl "Если это не попало на камеру, значит, этого не было."

# game/v11/scene2.rpy:471
translate russian v11_nora_chloe_hallway_e37346c3:

    # u "Hashtag Chloe caught in 4K. *Laughs*"
    u "Хэштег Хлоя видео в 4K. *Смеется*"

# game/v11/scene2.rpy:476
translate russian v11_nora_chloe_hallway_7ceb1447:

    # cl "I'd be mad at you for real if that ever happened."
    cl "Я бы на тебя разозлилась, если бы это случилось."

# game/v11/scene2.rpy:481
translate russian v11_nora_chloe_hallway_3601313f:

    # u "I'll just blame it on someone else and swoop in like the hero."
    u "Я просто свалю вину на кого-нибудь другого и ворвусь, как герой."

# game/v11/scene2.rpy:486
translate russian v11_nora_chloe_hallway_6cfa56a5:

    # cl "How Prince Charming of you."
    cl "Какой ты прекрасный принц."

# game/v11/scene2.rpy:491
translate russian v11_nora_chloe_hallway_cd253cc4:

    # u "My lady. *Laughs*"
    u "Моя леди. *Смеется*"

# game/v11/scene2.rpy:496
translate russian v11_nora_chloe_hallway_698ece09:

    # cl "I want to see if there's any new clothes I can get. I don't even know the latest European fashion trends."
    cl "Я хочу посмотреть, есть ли новая одежда, которую я могу купить. Я даже не знаю последних тенденций европейской моды."

# game/v11/scene2.rpy:501
translate russian v11_nora_chloe_hallway_3a308aad:

    # u "That's something you keep up with?"
    u "Это то, за чем ты следишь?"

# game/v11/scene2.rpy:506
translate russian v11_nora_chloe_hallway_b908b301:

    # cl "It is now."
    cl "Так и есть."

# game/v11/scene2.rpy:511
translate russian v11_nora_chloe_hallway_b54e4039:

    # u "Haha."
    u "Хаха."

# game/v11/scene2.rpy:516
translate russian v11_nora_chloe_hallway_1b3198e4:

    # cl "Just don't leave me all alone in Europe, there's hardly anyone I can hang out with."
    cl "Только не оставляй меня совсем одну в Европе, там вряд ли найдется кто-нибудь, с кем я могла бы пообщаться."

# game/v11/scene2.rpy:521
translate russian v11_nora_chloe_hallway_466f2f4d:

    # u "I get it, you just want to hang out with me, no need to make up excuses."
    u "Если ты просто хочешь потусоваться со мной, не нужно придумывать оправдания."

# game/v11/scene2.rpy:526
translate russian v11_nora_chloe_hallway_f8758378:

    # cl "You make good company... well, sometimes."
    cl "Ты составляешь хорошую компанию... иногда."

# game/v11/scene2.rpy:531
translate russian v11_nora_chloe_hallway_99a56b58:

    # u "Haha, well I won't be good company if I'm not there so I'm gonna go get packing."
    u "Хаха, я не буду хорошей компанией, если меня там не будет, так что пойду собираться."

# game/v11/scene2.rpy:536
translate russian v11_nora_chloe_hallway_155f5be0:

    # cl "Alright, it was good talking to you."
    cl "Ладно, было приятно с тобой поговорить."

# game/v11/scene2.rpy:541
translate russian v11_nora_chloe_hallway_9fb611a7:

    # u "You too."
    u "Мне тоже"

# game/v11/scene2.rpy:552
translate russian v11_nora_chloe_hallway_a7e11e98:

    # u "I wish, but I need to get started packing for the trip."
    u "Я бы хотел, но мне нужно начать собирать вещи для поездки."

# game/v11/scene2.rpy:557
translate russian v11_nora_chloe_hallway_19837ed6:

    # cl "Oh okay, well I'll see you later then."
    cl "Ну ладно, тогда увидимся позже."

# game/v11/scene2.rpy:562
translate russian v11_nora_chloe_hallway_104440d0:

    # u "See ya."
    u "Увидимся."

# game/v11/scene2.rpy:573
translate russian v11_nora_chloe_hallway_2a7a09ce:

    # u "*Sighs* Nevermind."
    u "Вздыхает* Неважно."

# game/v11/scene2.rpy:585
translate russian v11_nora_chloe_hallway_86728590:

    # u "Hey, cutie."
    u "Привет, милашка."

# game/v11/scene2.rpy:590
translate russian v11_nora_chloe_hallway_21a98069_1:

    # u "Hey Chloe."
    u "Привет, Хлоя."

# game/v11/scene2.rpy:596
translate russian v11_nora_chloe_hallway_2aac41e7:

    # cl "Oh, hey handsome. What's up?"
    cl "О, привет, красавчик. Как дела?"

# game/v11/scene2.rpy:601
translate russian v11_nora_chloe_hallway_477eeba5:

    # cl "Oh, hey [name]. What's up?"
    cl "О, привет [name]. Как дела?"

# game/v11/scene2.rpy:606
translate russian v11_nora_chloe_hallway_de2f2423:

    # u "Not much, I was just seeing who's all going all the trip."
    u "Не очень, просто посмотрел, кто собирается в поездку."

# game/v11/scene2.rpy:611
translate russian v11_nora_chloe_hallway_6c022094:

    # cl "It's a lot of people."
    cl "Очень много людей."

# game/v11/scene2.rpy:616
translate russian v11_nora_chloe_hallway_3f4e960e:

    # u "More than I expected. I hope everyone gets along. But I doubt it."
    u "Больше, чем я ожидал. Надеюсь, все поладят. Но я сомневаюсь в этом."

# game/v11/scene2.rpy:621
translate russian v11_nora_chloe_hallway_84eea86d:

    # cl "Haha, me too. You know, I have a few minutes before my next class. Wanna go outside and talk for a bit?"
    cl "Хаха, я тоже. Знаешь, у меня есть несколько минут до следующего занятия. Хочешь выйти на улицу и немного поговорить?"

# game/v11/scene2.rpy:631
translate russian v11_nora_chloe_hallway_37fe551f_1:

    # u "Sure."
    u "Конечно."

# game/v11/scene2.rpy:641
translate russian v11_nora_chloe_hallway_71d2b8be_1:

    # cl "Are you excited for the trip?"
    cl "Ты волнуешься перед поездкой?"

# game/v11/scene2.rpy:646
translate russian v11_nora_chloe_hallway_1b28a406_1:

    # u "Of course."
    u "Безусловно."

# game/v11/scene2.rpy:651
translate russian v11_nora_chloe_hallway_f9f95e51_1:

    # cl "I am too. I'm a little nervous knowing I'm going to have to be around some people that I don't really get along with, but I'll still have fun."
    cl "Я тоже. Я немного нервничаю, зная, что мне придется быть рядом с некоторыми людьми, с которыми я не лажу, но мне все равно будет весело."

# game/v11/scene2.rpy:656
translate russian v11_nora_chloe_hallway_ac82a12e_1:

    # u "Yeah, no reason not to enjoy yourself..."
    u "Да, нет причин не получать удовольствие..."

# game/v11/scene2.rpy:661
translate russian v11_nora_chloe_hallway_23edfa13_1:

    # cl "I wonder what will be the crazy thing that happens."
    cl "Интересно, что же такого безумного произойдет?"

# game/v11/scene2.rpy:666
translate russian v11_nora_chloe_hallway_09332657_1:

    # u "What do you mean?"
    u "Что ты имеешь в виду"

# game/v11/scene2.rpy:671
translate russian v11_nora_chloe_hallway_f7bcefcc_1:

    # cl "You know how every time you go on a trip something crazy always happens that everyone talks about? I went on a class trip a couple years ago and one of our classmates had a baby."
    cl "Ты знаешь, что каждый раз, когда ты отправляешься в путешествие, всегда происходит что-то сумасшедшее, о чем все говорят? Пару лет назад я ездила в классную поездку, и у одной из наших одноклассниц родился ребенок."

# game/v11/scene2.rpy:673
translate russian v11_nora_chloe_hallway_e1794fd5_1:

    # cl "*Laughs* She didn't even know she was pregnant."
    cl "*Смеется* Она даже не знала, что беременна."

# game/v11/scene2.rpy:678
translate russian v11_nora_chloe_hallway_7d966cb0_1:

    # u "*Chuckles* That's horrible."
    u "*Хихикает* Это ужасно."

# game/v11/scene2.rpy:683
translate russian v11_nora_chloe_hallway_6dbb700d_1:

    # cl "Yeah, it was pretty bad at the time. I guess we'll see what people will talk about after Europe..."
    cl "Да, в то время все было очень плохо. Думаю, мы посмотрим, о чем люди будут говорить после Европы..."

# game/v11/scene2.rpy:688
translate russian v11_nora_chloe_hallway_bc7666e9_1:

    # u "Maybe you'll trip over the Queen's gown while we're in London... That'd be funny now and a few years later."
    u "Может быть, ты споткнешься о платье королевы, когда мы будем в Лондоне... Это было бы забавно сейчас и спустя несколько лет."

# game/v11/scene2.rpy:693
translate russian v11_nora_chloe_hallway_c24522e3_1:

    # cl "If it isn't caught on camera it never happened."
    cl "Если это не попало на камеру, значит, этого не было."

# game/v11/scene2.rpy:698
translate russian v11_nora_chloe_hallway_e37346c3_1:

    # u "Hashtag Chloe caught in 4K. *Laughs*"
    u "Хэштег Хлоя видео в 4K. *Смеется*"

# game/v11/scene2.rpy:703
translate russian v11_nora_chloe_hallway_7ceb1447_1:

    # cl "I'd be mad at you for real if that ever happened."
    cl "Я бы на тебя разозлилась, если бы это случилось."

# game/v11/scene2.rpy:708
translate russian v11_nora_chloe_hallway_3601313f_1:

    # u "I'll just blame it on someone else and swoop in like the hero."
    u "Я просто свалю вину на кого-нибудь другого и ворвусь, как герой."

# game/v11/scene2.rpy:713
translate russian v11_nora_chloe_hallway_6cfa56a5_1:

    # cl "How Prince Charming of you."
    cl "Какой ты прекрасный принц."

# game/v11/scene2.rpy:718
translate russian v11_nora_chloe_hallway_cd253cc4_1:

    # u "My lady. *Laughs*"
    u "Моя леди. *Смеется*"

# game/v11/scene2.rpy:723
translate russian v11_nora_chloe_hallway_698ece09_1:

    # cl "I want to see if there's any new clothes I can get. I don't even know the latest European fashion trends."
    cl "Я хочу посмотреть, есть ли новая одежда, которую я могу купить. Я даже не знаю последних тенденций европейской моды."

# game/v11/scene2.rpy:728
translate russian v11_nora_chloe_hallway_3a308aad_1:

    # u "That's something you keep up with?"
    u "Это то, за чем ты следишь?"

# game/v11/scene2.rpy:733
translate russian v11_nora_chloe_hallway_b908b301_1:

    # cl "It is now."
    cl "Так и есть."

# game/v11/scene2.rpy:738
translate russian v11_nora_chloe_hallway_b54e4039_1:

    # u "Haha."
    u "Хаха"

# game/v11/scene2.rpy:743
translate russian v11_nora_chloe_hallway_1b3198e4_1:

    # cl "Just don't leave me all alone in Europe, there's hardly anyone I can hang out with."
    cl "Только не оставляй меня совсем одну в Европе, там вряд ли найдется кто-нибудь, с кем я могла бы пообщаться."

# game/v11/scene2.rpy:748
translate russian v11_nora_chloe_hallway_466f2f4d_1:

    # u "I get it, you just want to hang out with me, no need to make up excuses."
    u "Если ты просто хочешь потусоваться со мной, не нужно придумывать оправдания."

# game/v11/scene2.rpy:753
translate russian v11_nora_chloe_hallway_73888434:

    # cl "You make good company... well sometimes."
    cl "Ты составляешь хорошую компанию... иногда."

# game/v11/scene2.rpy:758
translate russian v11_nora_chloe_hallway_99a56b58_1:

    # u "Haha, well I won't be good company if I'm not there so I'm gonna go get packing."
    u "Хаха, я не буду хорошей компанией, если меня там не будет, так что пойду собираться."

# game/v11/scene2.rpy:764
translate russian v11_nora_chloe_hallway_451391af:

    # cl "I need to run but I can't wait to spend some more time with you in Europe."
    cl "Мне нужно идти, не могу дождаться, чтобы провести с тобой еще немного времени в Европе."

# game/v11/scene2.rpy:769
translate russian v11_nora_chloe_hallway_155f5be0_1:

    # cl "Alright, it was good talking to you."
    cl "Ладно, было приятно с тобой поговорить."

# game/v11/scene2.rpy:776
translate russian v11_nora_chloe_hallway_eec6a617:

    # u "See you later, beautiful."
    u "Увидимся позже, красавица."

# game/v11/scene2.rpy:781
translate russian v11_nora_chloe_hallway_9fb611a7_1:

    # u "You too."
    u "Мне тоже."

# game/v11/scene2.rpy:792
translate russian v11_nora_chloe_hallway_a7e11e98_1:

    # u "I wish, but I need to get started packing for the trip."
    u "Я бы хотел, но мне нужно начать собирать вещи для поездки."

# game/v11/scene2.rpy:797
translate russian v11_nora_chloe_hallway_19837ed6_1:

    # cl "Oh okay, well I'll see you later then."
    cl "Окей, тогда увидимся позже"

# game/v11/scene2.rpy:802
translate russian v11_nora_chloe_hallway_104440d0_1:

    # u "See ya."
    u "Увдимся"

translate russian strings:

    # game/v11/scene2.rpy:66
    old "I'll be there"
    new "Я буду там"

    # game/v11/scene2.rpy:66
    old "Have a good time"
    new "Хорошо провести время"

    # game/v11/scene2.rpy:119
    old "Don't defend Chloe"
    new "Не защищать Хлою"

    # game/v11/scene2.rpy:252
    old "Mellow"
    new "Спокойный"

    # game/v11/scene2.rpy:252
    old "Wild"
    new "Дикий"

    # game/v11/scene2.rpy:271
    old "Hanging with the girls"
    new "Тусоваться с девчонками"

    # game/v11/scene2.rpy:271
    old "Sleeping"
    new "Спать"

    # game/v11/scene2.rpy:336
    old "Say something"
    new "Скажи что-нибудь"

    # game/v11/scene2.rpy:358
    old "Give up"
    new "Сдаться"

