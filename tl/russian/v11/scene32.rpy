# TODO: Translation updated at 2021-11-10 00:46

# game/v11/scene32.rpy:10
translate russian v11_emily_sidewalk_90a13b04:

    # u "Hey Emily, haven't seen you the whole time we've been here."
    u "Привет, Эмили, я не видел тебя всё это время, пока мы были здесь."

# game/v11/scene32.rpy:15
translate russian v11_emily_sidewalk_92814b2d:

    # em "That's the point. Do you not remember our talk?"
    em "В том-то и дело. Ты не помнишь наш разговор?"

# game/v11/scene32.rpy:20
translate russian v11_emily_sidewalk_9e2e4cd5:

    # u "Yeah I remember, but-"
    u "Да, я помню, но..."

# game/v11/scene32.rpy:25
translate russian v11_emily_sidewalk_b213e5e7:

    # em "Then you know why we haven't seen each other."
    em "Тогда ты знаешь, почему мы не виделись."

# game/v11/scene32.rpy:35
translate russian v11_emily_sidewalk_16caa92e:

    # u "(She really doesn't want to talk to me. Oh, well.)"
    u "Она не хочет со мной разговаривать. Ну и ладно.)"

