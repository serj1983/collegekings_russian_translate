# TODO: Translation updated at 2021-11-10 00:46
# Перевёл: serj1983 (post.serj@gmail.com)

# game/v11/scene15.rpy:10
translate russian v11_nora_airport_convo_e9c96e1e:

    # u "(Oh...)"
    u "(Ох...)"

# game/v11/scene15.rpy:15
translate russian v11_nora_airport_convo_773cf37f:

    # charli "*Inaudible*"
    charli "*Непонятно*"

# game/v11/scene15.rpy:20
translate russian v11_nora_airport_convo_8e3039b2:

    # no "*Inaudible*"
    no "*Непонятно*"

# game/v11/scene15.rpy:25
translate russian v11_nora_airport_convo_38daa07a:

    # imre "*Whisper* Being nice to lay the pipe, what a fucking simp."
    imre "*Шепот* Быть милым, чтобы проложить трубу, какой гребаный простак."

# game/v11/scene15.rpy:27
translate russian v11_nora_airport_convo_68ef16d2:

    # u "(I was thinking the same thing!)"
    u "(Я думал о том же самом!)"

# game/v11/scene15.rpy:37
translate russian v11_nora_airport_convo_63a7bd23:

    # u "Uh, excuse me."
    u "Прошу прощения."

# game/v11/scene15.rpy:42
translate russian v11_nora_airport_convo_38d1a497:

    # charli "Hey champ, you trying to talk to Nora?"
    charli "Эй, чемпион, ты пытаешься поговорить с Норой?"

# game/v11/scene15.rpy:47
translate russian v11_nora_airport_convo_481fd912:

    # u "Well... yeah. I saw what happened on the plane and we've already had some conversations about this whole thing, so I wanna see if she's good."
    u "Ну... да. Я видел, что случилось в самолете, и у нас уже было несколько разговоров обо всем этом, так что я хочу посмотреть, все ли с ней в порядке."

# game/v11/scene15.rpy:52
translate russian v11_nora_airport_convo_034a695d:

    # charli "Mhmm... I see. Well... Please try to understand that she's under a lot of pressure right now. One can assume she doesn't want to be surrounded or overwhelmed."
    charli "Ммм... Понятно. Ну... Пожалуйста, постарайтесь понять, что она сейчас находится под большим давлением. Можно предположить, что она не хочет быть окруженной или подавленной."

# game/v11/scene15.rpy:53
translate russian v11_nora_airport_convo_0d70c28a:

    # charli "I was already gonna talk to her, so just don't worry about it. Have a good time. I'll make sure she's okay, you don't have to worry."
    charli "Я уже собирался поговорить с ней, так что не волнуйся об этом. Хорошо проведи время. Я прослежу, чтобы с ней все было в порядке, можешь не волноваться."

# game/v11/scene15.rpy:58
translate russian v11_nora_airport_convo_4af72cfd:

    # u "I think with the situation being so stressful and the fact that I've already had a talk with her about this, it'd be good if I spoke with her."
    u "Думаю, учитывая, что ситуация настолько стрессовая, и тот факт, что я уже говорил с ней об этом, будет хорошо, если я поговорю с ней."

# game/v11/scene15.rpy:59
translate russian v11_nora_airport_convo_b37c46f5:

    # u "Between the frat situations, this whole Europe trip, her and Chris... I'm pretty involved already."
    u "Между ситуациями в братстве, всей этой поездкой в Европу, ею и Крисом... Я уже достаточно вовлечен."

# game/v11/scene15.rpy:60
translate russian v11_nora_airport_convo_e20fe213:

    # u "No offense, but I doubt you have weeks of drama under control without a clear understanding of everyone's perspective."
    u "Без обид, но я сомневаюсь, что у тебя неделя драмы под контролем без четкого понимания перспективы каждого."

# game/v11/scene15.rpy:65
translate russian v11_nora_airport_convo_5c6b4dfa:

    # charli "That's true, but although I lack everyone else's perspective I know Nora very well."
    charli "Это правда, но хотя мне не хватает чужой точки зрения, я очень хорошо знаю Нору."

# game/v11/scene15.rpy:66
translate russian v11_nora_airport_convo_e9126ded:

    # charli "And to be frank [name], that's the only perspective I'm concerned about right now. I mean think about it. Sure it's reasonable for everything and all sides to be out in the air, but look at her."
    charli "И если быть откровенным [name], это единственная перспектива, которая меня сейчас волнует. Я имею в виду, подумай об этом. Конечно, это разумно для всего и всех сторон быть в воздухе, но посмотрите на нее"

# game/v11/scene15.rpy:74
translate russian v11_nora_airport_convo_bb53430b:

    # charli "She couldn't care less about any of that right now. I know during my own times of frustration I don't want to hear what anyone has to say unless they're agreeing with me. *Chuckles*"
    charli "Сейчас ей на все это наплевать. Я знаю, что в периоды разочарований я не хочу слушать, что кто-то говорит, если только он не согласен со мной. *Хихикает*"

# game/v11/scene15.rpy:75
translate russian v11_nora_airport_convo_a8b7adb8:

    # charli "Look, she's expecting me to come back, but if you'd like me to step aside and let you speak with her, I will."
    charli "Послушай, она ждет, что я вернусь, но если ты хочешь, чтобы я был в стороне и позволил поговорить с ней, я сделаю это."

# game/v11/scene15.rpy:80
translate russian v11_nora_airport_convo_3b739a66:

    # u "*Sighs* If she's expecting you then I'm not going to try to get in the way."
    u "*Вздох* Если она ждет тебя, то я не буду мешать."

# game/v11/scene15.rpy:85
translate russian v11_nora_airport_convo_70a9f126:

    # charli "Are you sure? I know she's your friend, I'm not trying to be \"that guy\"."
    charli "Ты уверен? Я знаю, что она твоя подруга, я не пытаюсь быть \"тем парнем\"."

# game/v11/scene15.rpy:90
translate russian v11_nora_airport_convo_5af2bb70:

    # u "Don't worry about it."
    u "Не беспокойтесь об этом."

# game/v11/scene15.rpy:95
translate russian v11_nora_airport_convo_907136b8:

    # charli "You sure?"
    charli "Ты уверен?"

# game/v11/scene15.rpy:100
translate russian v11_nora_airport_convo_505a3f68:

    # u "Yeah."
    u "Да."

# game/v11/scene15.rpy:105
translate russian v11_nora_airport_convo_0724389c:

    # charli "Okay then, if I feel she needs you or I can't help her I'll send her your way."
    charli "Хорошо, тогда, если я почувствую, что она нуждается в тебе или я не смогу ей помочь, я отправлю её к тебе."

# game/v11/scene15.rpy:110
translate russian v11_nora_airport_convo_ee01f9b9:

    # u "Thanks."
    u "Спасибо."

# game/v11/scene15.rpy:120
translate russian v11_nora_airport_convo_773cf37f_1:

    # charli "*Inaudible*"
    charli "*Непонятно*"

# game/v11/scene15.rpy:125
translate russian v11_nora_airport_convo_0e21f62f:

    # u "(Okay, now I'm kinda getting pissed. He doesn't even know her like that.)"
    u "(Ладно, теперь я начинаю злиться. Он даже не знает её такой.)"

# game/v11/scene15.rpy:126
translate russian v11_nora_airport_convo_c9955893:

    # u "(It's obvious he's just trying to get close to her while she's upset so she'll let him fuck. I officially don't like this guy. Acting all nice and shit.)"
    u "(Очевидно, что он просто пытается сблизиться с ней, пока она расстроена, чтобы она позволила ему трахаться. Мне не нравится этот парень. Ведет себя хорошо и всё такое)."

# game/v11/scene15.rpy:141
translate russian v11_nora_airport_convo_85152a9e:

    # u "You know man, there's a lot of girls here at school, is there a reason you keep going for the ones I'm closest to?"
    u "Знаешь, парень, в школе много девушек, есть причина, по которой ты всё время выбираешь тех, с кем я ближе всего?"

# game/v11/scene15.rpy:146
translate russian v11_nora_airport_convo_f37d1579:

    # charli "*Smirks* I don't think you understand what's going on."
    charli "*Ухмыляется* Не думаю, что ты понимаешь, что происходит."

# game/v11/scene15.rpy:151
translate russian v11_nora_airport_convo_dc631653:

    # u "What's going on is you're undermining me in front of the people I'm closest to. And I don't fuck with that."
    u "Происходит то, что ты подрываешь меня перед самыми близкими мне людьми. И мне это не по барабану."

# game/v11/scene15.rpy:156
translate russian v11_nora_airport_convo_0f41161c:

    # charli "Pfft. I was just trying to be friendly. But if you wanna make this some pity, jealousy feud, sure. Go ahead. Just know that we could've been good friends and your life would be a whole lot easier."
    charli "Пфф. Я просто пытался быть дружелюбным. Но если ты хочешь сделать из этого какую-то вражду на почве жалости и ревности, конечно. Давай. Просто знай, что мы могли бы быть хорошими друзьями, и твоя жизнь была бы намного проще."

# game/v11/scene15.rpy:161
translate russian v11_nora_airport_convo_586648a9:

    # u "*Whisper* Fucking asshole!"
    u "*Шёпот* Ёбаный мудак!"

# game/v11/scene15.rpy:166
translate russian v11_nora_airport_convo_92d7d6bc:

    # ri "*Whisper* [name]! Why'd you say that to him?"
    ri "Шёпот* [name]! Зачем ты ему это сказал?"

# game/v11/scene15.rpy:171
translate russian v11_nora_airport_convo_361c8c34:

    # u "Cause it's true, he's pissing me off."
    u "Потому что, он меня бесит."

# game/v11/scene15.rpy:176
translate russian v11_nora_airport_convo_00bd2189:

    # ri "He didn't even do anything wrong. You know he's gay, right?"
    ri "Он даже не сделал ничего плохого. Ты ведь знаешь, что он гей?"

# game/v11/scene15.rpy:181
translate russian v11_nora_airport_convo_a033be3c:

    # u "No, I didn't know that, but I still don't fuck with him. He just comes out of nowhere and all of a sudden he's buddied up to everyone telling me to step back. Like what the fuck?"
    u "Нет, я этого не знал, но я все равно с ним не общаюсь. Он просто появился из ниоткуда и вдруг со всеми подружился. Какого хрена?"

# game/v11/scene15.rpy:186
translate russian v11_nora_airport_convo_6730ef15:

    # ri "Well, I've gotten to know him pretty well, [name], he's really not that bad. At least try and get to know him, alright?"
    ri "Ну, я узнал его довольно хорошо, [name], он действительно не так уж плох. По крайней мере, постарайся узнать его получше, хорошо?"

# game/v11/scene15.rpy:187
translate russian v11_nora_airport_convo_d2228e51:

    # ri "I can get why you may feel the way you do, but get all the info before you pop off on someone like that."
    ri "Я могу понять, почему ты можешь так себя чувствовать, но узнаю всю информацию, прежде чем набраываться на кого-то."

# game/v11/scene15.rpy:192
translate russian v11_nora_airport_convo_1695b0ce:

    # u "*Sighs* If you say so."
    u "*Вздох* Как скажешь."

# game/v11/scene15.rpy:197
translate russian v11_nora_airport_convo_ab8f07e5:

    # ri "Now be happy! We're on vacation, grumpy man."
    ri "А теперь будь счастлив! Мы в отпуске, ворчун."

# TODO: Translation updated at 2023-06-21 15:57

# game/v11/scene15.rpy:156
translate russian v11_nora_airport_convo_9fe4d1d5:

    # charli "Pfft. I was just trying to be friendly. But if you wanna make this some petty, jealousy feud, sure. Go ahead."
    charli "Пфф. Я просто пытался быть дружелюбным. Но если ты хочешь превратить это в какую-то мелкую ревнивую вражду, конечно. Продолжай."

# game/v11/scene15.rpy:158
translate russian v11_nora_airport_convo_a56faefe:

    # charli "Just know that we could've been good friends and your life would be a whole lot easier."
    charli "Просто знай, что мы могли бы быть хорошими друзьями, и твоя жизнь была бы намного проще."

