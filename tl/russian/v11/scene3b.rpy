# TODO: Translation updated at 2021-11-10 00:46
# Перевёл: serj1983 (post.serj@gmail.com)

# game/v11/scene3b.rpy:13
translate russian v11_cafe_with_riley_fcfea652:

    # u "Hello?"
    u "Алло?"

# game/v11/scene3b.rpy:18
translate russian v11_cafe_with_riley_63d279d9:

    # ri "Hey [name], it's Riley. I'm over here at the cafe on Stevenson trying to clear my head about some things, and I thought it'd be nice to just talk to someone about it. Mind coming down?"
    ri "Привет [name], это Райли. Я здесь, в кафе на Стивенсон, пытаюсь прояснить некоторые вещи, и я подумала, что было бы неплохо поговорить с кем-нибудь об этом. Не против спуститься вниз?"

# game/v11/scene3b.rpy:23
translate russian v11_cafe_with_riley_4f086ef9:

    # u "No I don't mind at all, I'll be there soon."
    u "Ссовсем не против, я скоро буду там."

# game/v11/scene3b.rpy:28
translate russian v11_cafe_with_riley_ac04ba6e:

    # ri "Okay cool, thanks. See ya."
    ri "Ладно, круто, спасибо. Увидимся."

# game/v11/scene3b.rpy:33
translate russian v11_cafe_with_riley_c5d4568d:

    # u "Later, Riley."
    u "Позже, Райли."

# game/v11/scene3b.rpy:39
translate russian v11_cafe_with_riley_fcfea652_1:

    # u "Hello?"
    u "Привет?"

# game/v11/scene3b.rpy:44
translate russian v11_cafe_with_riley_63d279d9_1:

    # ri "Hey [name], it's Riley. I'm over here at the cafe on Stevenson trying to clear my head about some things, and I thought it'd be nice to just talk to someone about it. Mind coming down?"
    ri "Привет [name], это Райли. Я здесь, в кафе на Стивенсон, пытаюсь прояснить некоторые вещи, и я подумала, что было бы неплохо поговорить с кем-нибудь об этом. Не против спуститься вниз?"

# game/v11/scene3b.rpy:49
translate russian v11_cafe_with_riley_4f086ef9_1:

    # u "No I don't mind at all, I'll be there soon."
    u "Совсем не против, я скоро буду там."

# game/v11/scene3b.rpy:54
translate russian v11_cafe_with_riley_ac04ba6e_1:

    # ri "Okay cool, thanks. See ya."
    ri "Ладно, круто, спасибо. Увидимся."

# game/v11/scene3b.rpy:59
translate russian v11_cafe_with_riley_c5d4568d_1:

    # u "Later, Riley."
    u "Позже, Райли."

# game/v11/scene3b.rpy:84
translate russian v11_cafe_with_riley_8686e81c:

    # u "Hey."
    u "Привет."

# game/v11/scene3b.rpy:89
translate russian v11_cafe_with_riley_a40af0da:

    # ri "Hey."
    ri "Привет."

# game/v11/scene3b.rpy:94
translate russian v11_cafe_with_riley_485f63b5:

    # u "So, how do you wanna do this? A little small talk first or just dive right in?"
    u "Итак, как ты хочешь это сделать? Сначала немного светской беседы или сразу погружаемся в работу?"

# game/v11/scene3b.rpy:99
translate russian v11_cafe_with_riley_21e4eccb:

    # ri "Haha, I don't wanna keep you forever so I'll just say what's on my mind... I'm kinda nervous about the trip, actually."
    ri "Хаха, я не хочу тебя долго задерживать, поэтому я скажу то, что у меня на уме... Вообще-то, я немного нервничаю из-за поездки."

# game/v11/scene3b.rpy:104
translate russian v11_cafe_with_riley_48c99239:

    # u "Why?"
    u "Почему?"

# game/v11/scene3b.rpy:109
translate russian v11_cafe_with_riley_722c1512:

    # ri "Wait, let me rephrase that... I'm kinda nervous about flying to Europe."
    ri "Подождите, позвольте мне перефразировать... Я немного нервничаю из-за полета в Европу."

# game/v11/scene3b.rpy:114
translate russian v11_cafe_with_riley_4433d913:

    # u "What's got you so freaked out?"
    u "Что тебя так напугало?"

# game/v11/scene3b.rpy:119
translate russian v11_cafe_with_riley_9f3a1855:

    # ri "I've never flown before and I don't know what to expect. Will I get sick? Are there going to be a lot of issues with the plane?"
    ri "Я никогда раньше не летала и не знаю, чего ожидать. Заболею ли я? Будут ли проблемы с самолетом?"

# game/v11/scene3b.rpy:120
translate russian v11_cafe_with_riley_ad547709:

    # ri "I literally know nothing except for the things I've seen in movies. And in movies, there's no middle man. It's either the plane crashes or people are enjoying themselves in first class."
    ri "Я буквально ничего не знаю, кроме того, что видел в фильмах. А в фильмах нет посредника. Либо самолет падает, либо люди наслаждаются в первом классе."

# game/v11/scene3b.rpy:121
translate russian v11_cafe_with_riley_13918ced:

    # ri "And although one of those would be nice, it won't be happening... So I really hope the plane doesn't crash."
    ri "И хотя одно из них было бы неплохо, этого не произойдет... Так что я очень надеюсь, что самолет не разобьется."

# game/v11/scene3b.rpy:129
translate russian v11_cafe_with_riley_79067112:

    # u "Well... Planes don't crash often, but there's usually a lot of turbulence which makes the plane shake pretty hard. That part is a little scary."
    u "Ну... Самолеты падают не часто, обычно бывает сильная турбулентность, которая заставляет самолет сильно трястись. Это немного пугает."

# game/v11/scene3b.rpy:130
translate russian v11_cafe_with_riley_41f33989:

    # u "It's mostly stable in first class, but in the regular area where we'll be, it can get kinda bad."
    u "Он в основном стабилен в первом классе, но в обычной области, где мы будем, это может быть немного плохо."

# game/v11/scene3b.rpy:135
translate russian v11_cafe_with_riley_bfaa40a4:

    # ri "That doesn't make me feel any better..."
    ri "От этого мне не легче..."

# game/v11/scene3b.rpy:140
translate russian v11_cafe_with_riley_b2109baa:

    # u "Haha, I'm just messing with you. It's really not that bad. You probably won't even notice we're moving."
    u "Хаха, я просто прикалываюсь над тобой. Всё действительно не так уж плохо. Ты, даже не заметишь, что мы прилетим."

# game/v11/scene3b.rpy:145
translate russian v11_cafe_with_riley_fe691d49:

    # ri "Amber also said the turbulence makes the plane shake a lot... Now I feel like you're just trying to make me feel better by hiding the truth."
    ri "Эмбер сказала, что из-за турбулентности самолет сильно трясет... Теперь я чувствую, что ты просто пытаешься заставить меня чувствовать себя лучше, скрывая правду."

# game/v11/scene3b.rpy:150
translate russian v11_cafe_with_riley_d483f6a8:

    # u "*Laughs* Well, the truth is I was trying to scare you with something that would be believable. Sounds like Amber had the same idea."
    u "*Смеется* По правде говоря, я пытался напугать тебя чем-то правдоподобным. Похоже, у Эмбер была та же идея."

# game/v11/scene3b.rpy:151
translate russian v11_cafe_with_riley_da8ea1c2:

    # u "Just download some movies on your phone to watch and for most of it, you'll probably be asleep anyway... You really have nothing to worry about."
    u "Просто скачай несколько фильмов на свой телефон, чтобы было что смотреть, или большую часть времени, все равно будешь спать... Тебе не о чем беспокоиться."

# game/v11/scene3b.rpy:152
translate russian v11_cafe_with_riley_0473c8f4:

    # u "I'm pretty sure Imre is scared of heights, but he has no problem with the plane."
    u "Вот Имре боится высоты, но у него нет проблем с самолетом."

# game/v11/scene3b.rpy:158
translate russian v11_cafe_with_riley_bc038785:

    # u "*Chuckles* If you need someone to hold your hand for the whole flight I will, but you really have nothing to worry about."
    u "*Хихикает* Если тебе нужно, чтобы кто-то держал тебя за руку весь полет, я это сделаю, так что не беспокойся."

# game/v11/scene3b.rpy:159
translate russian v11_cafe_with_riley_73b86659:

    # u "I've been on multiple planes and know many others that have flown, and we've never had any problems. I just watch movies and sleep the whole time."
    u "Я летал на нескольких самолетах и знаю многих, которые летали, и у нас никогда не было проблем. Я просто смотрю фильмы и все время сплю."

# game/v11/scene3b.rpy:164
translate russian v11_cafe_with_riley_ed5df938:

    # ri "Be really honest though, what's the worst thing about it?"
    ri "Будь честен, что самое худшее?"

# game/v11/scene3b.rpy:169
translate russian v11_cafe_with_riley_1c8b7795:

    # u "Well, the elevation messes with your ears a bit. Kinda feels like you're underwater. You get rid of it by yawning, but your ears sort of pop. For myself and most people that's the worst part."
    u "Ну, высота немного мешает ушам. Такое ощущение, что ты находишься под водой. Ты избавляешься от этого, зевая, у тебя как бы закладывает уши. Для меня и большинства людей это самое худшее."

# game/v11/scene3b.rpy:174
translate russian v11_cafe_with_riley_ac18688a:

    # ri "Your ears pop?! Does it hurt?"
    ri "У тебя закладывало уши?! Это больно?"

# game/v11/scene3b.rpy:179
translate russian v11_cafe_with_riley_a03e22ac:

    # u "Haha, no Riley, it doesn't hurt. It's just an unusual feeling. I'm sure you've popped your knuckles before. It's like that but with your ear."
    u "Ха-ха, нет, Райли, это не больно. Это просто необычное чувство. Я уверен, что ты и раньше хрустела костяшками пальцев. Это похоже на то, но с твоим ухом."

# game/v11/scene3b.rpy:184
translate russian v11_cafe_with_riley_62ac0c70:

    # ri "Okay, I guess that doesn't seem so bad."
    ri "Ладно, думаю, это не так уж и плохо."

# game/v11/scene3b.rpy:189
translate russian v11_cafe_with_riley_b4ad2879:

    # u "See? Nothing to stress over."
    u "Видишь? Не из-за чего переживать."

# game/v11/scene3b.rpy:194
translate russian v11_cafe_with_riley_bdc341b1:

    # ri "Alright, I guess I shouldn't worry about it. I am gonna talk to Amber though because I know she tried to scare me with her \"make sure you tuck your feet under the seat so you don't fall out\". *Chuckles*"
    ri "Ладно, думаю, мне не стоит беспокоиться об этом. Я собираюсь поговорить с Эмбер, потому что я знаю, что она пыталась напугать меня своим \"убедись, что засунула ноги под сиденье, чтобы не выпасть\". *Хихикает*"

# game/v11/scene3b.rpy:199
translate russian v11_cafe_with_riley_4b3eb010:

    # u "*Laughs* Yeah, talking to Amber about your fears probably wasn't a good idea. If I had never flown before and talked to Amber before my first time, I'd be nervous too. She has that kind of power."
    u "*Смеется* Да, говорить с Эмбер о своих страхах, вероятно, было не очень хорошей идеей. Если бы я никогда раньше не летал и не разговаривал с Эмбер до моего первого раза, я бы тоже нервничал. У нее есть такая сила."

# game/v11/scene3b.rpy:204
translate russian v11_cafe_with_riley_17ced004:

    # ri "We should call her Maleficent... *Chuckles*"
    ri "Мы должны назвать её Малефисентой... *Хихикает*"

# game/v11/scene3b.rpy:209
translate russian v11_cafe_with_riley_ff499cd4:

    # u "What's maleficent?"
    u "Что за Малефисента?"

# game/v11/scene3b.rpy:214
translate russian v11_cafe_with_riley_a449cba2:

    # ri "Oh my god, look it up later."
    ri "О боже, посмотри потом."

# game/v11/scene3b.rpy:219
translate russian v11_cafe_with_riley_41b9ee1a:

    # u "*Chuckles* Okay? Hey, I know a way you could get over your fear of flying pretty easily."
    u "Хихикает* Хорошо? Я знаю способ, с помощью которого ты сможешь легко преодолеть свой страх перед полетами."

# game/v11/scene3b.rpy:224
translate russian v11_cafe_with_riley_ca130271:

    # ri "Really? How?"
    ri "Правда? Какой?"

# game/v11/scene3b.rpy:229
translate russian v11_cafe_with_riley_2623d696:

    # u "You should try out that little indoor skydiving thing."
    u "Ты должна попробовать прыгнуть с парашютом в закрытом помещении."

# game/v11/scene3b.rpy:234
translate russian v11_cafe_with_riley_7949f754:

    # ri "What?"
    ri "Что?"

# game/v11/scene3b.rpy:239
translate russian v11_cafe_with_riley_35e3b2f4:

    # u "You know, the thing with the giant air tube that lets you float?"
    u "Знаешь, ту штуку с гигантской воздушной трубкой, которая позволяет тебе плавать?"

# game/v11/scene3b.rpy:244
translate russian v11_cafe_with_riley_736c6c0f:

    # ri "Oh, right! I saw a commercial about that, I did think it kinda looked fun."
    ri "О, точно! Я видела рекламу, и мне показалось, что это выглядит забавно."

# game/v11/scene3b.rpy:249
translate russian v11_cafe_with_riley_6400f3c1:

    # u "Well, it's safe and probably more stressful than an actual plane ride."
    u "Что ж, это безопасно и, вероятно, более напряженно, чем настоящий полёт на самолете."

# game/v11/scene3b.rpy:254
translate russian v11_cafe_with_riley_a6581dea:

    # ri "Maybe less stressful for you. *Laughs*"
    ri "Может быть, для тебя это будет менее напряженно. *Смеется*"

# game/v11/scene3b.rpy:264
translate russian v11_cafe_with_riley_18930abe:

    # charli "Hey [name]! Is this one of the close friends you were telling me about? I love your hair! Is that color natural?"
    charli "Эй [name]! Это один из тех близких друзей, о которых ты мне рассказывал? Мне нравятся твои волосы! Этот цвет настоящий?"

# game/v11/scene3b.rpy:269
translate russian v11_cafe_with_riley_a2c4ca94:

    # ri "Oh, thank you so much! I'm Riley. And yeah, haha. it's natural... You're a friend of [name]?"
    ri "О, большое вам спасибо! Я Райли. И да, ха-ха. это настоящий... Вы друг [name]?"

# game/v11/scene3b.rpy:274
translate russian v11_cafe_with_riley_15ce87e8:

    # charli "Yea- well... I don't know, actually. *Chuckles* Would you say we're friends [name]?"
    charli "Да- ну... Я не знаю, вообще-то. *Хихикает* Ты бы сказал, что мы друзья [name]?"

# game/v11/scene3b.rpy:281
translate russian v11_cafe_with_riley_b222b29c:

    # u "Haha, maybe not yet, I mean we just met this morning..."
    u "Хаха, может, еще нет, мы же только сегодня утром познакомились..."

# game/v11/scene3b.rpy:288
translate russian v11_cafe_with_riley_a77065ae:

    # u "Sure, of course. Charli and I actually just met this morning."
    u "Конечно, конечно. Мы с Чарли только сегодня утром познакомились."

# game/v11/scene3b.rpy:293
translate russian v11_cafe_with_riley_40660b45:

    # ri "Well, it's nice to meet you, Charli. Love your energy, so positive."
    ri "Приятно познакомиться с тобой, Чарли. Мне нравится ваша энергия, такой позитивный."

# game/v11/scene3b.rpy:298
translate russian v11_cafe_with_riley_61d92fd9:

    # charli "It's my pleasure, really. I was just telling [name] that I needed to find some friends on campus and here you are. *Chuckles*"
    charli "Мне очень приятно, правда. Я как раз говорил [name], что мне нужно найти друзей в кампусе, и вот ты здесь. *Хихикает*"

# game/v11/scene3b.rpy:303
translate russian v11_cafe_with_riley_22824727:

    # ri "Haha, yes. Here I am. *Chuckles*"
    ri "Хаха, да. Вот и я. *Хихикает"

# game/v11/scene3b.rpy:308
translate russian v11_cafe_with_riley_b59b263a:

    # u "Charli here is actually going on the Europe trip with us."
    u "Чарли на самом деле собирается с нами в поездку по Европе."

# game/v11/scene3b.rpy:313
translate russian v11_cafe_with_riley_a22e33b5:

    # ri "Oh yayyy! Now I won't be stuck with just [name]. *Chuckles*"
    ri "О, ура! Теперь я не буду зацикливаться только на [name]. *Хихикает*"

# game/v11/scene3b.rpy:318
translate russian v11_cafe_with_riley_cb51324b:

    # u "Wowww... *Chuckles*"
    u "Вау... *Хихикает*"

# game/v11/scene3b.rpy:323
translate russian v11_cafe_with_riley_477fb607:

    # charli "[name] you must be one hell of a guy considering you're surrounded by all these amazing people..."
    charli "[name], ты, должно быть, чертовски хороший парень, раз вас окружают такие удивительные люди..."

# game/v11/scene3b.rpy:328
translate russian v11_cafe_with_riley_4cc21c21:

    # ri "\"Amazing\"? Definitely not. *Chuckles*"
    ri "\'Удивительные\"? Да ладно тебе. *Хихикает*"

# game/v11/scene3b.rpy:333
translate russian v11_cafe_with_riley_ce575d5b:

    # u "(\"Amazing\"...)"
    u "(\"Удивительные\"...)"

# game/v11/scene3b.rpy:338
translate russian v11_cafe_with_riley_6639c906:

    # ri "How come I've never bumped into you before?"
    ri "Как получилось, что я никогда не сталкивалась с тобой раньше?"

# game/v11/scene3b.rpy:343
translate russian v11_cafe_with_riley_2376aa9d:

    # charli "I'm actually an exchange student and I currently stay off campus. But honestly I would have made more of an effort to meet people on campus if I had known you guys were here. *Chuckles*"
    charli "На самом деле я студент по обмену и сейчас живу за пределами кампуса. Но, честно говоря, я бы приложил больше усилий, чтобы познакомиться с людьми в кампусе, если бы знал, что вы, ребята, здесь. *Хихикает*"

# game/v11/scene3b.rpy:348
translate russian v11_cafe_with_riley_7516831a:

    # ri "Ahh... that would explain a lot... Well, I'm really glad you're going on the trip with us. That'll give us the perfect chance to get to know each other."
    ri "Ааа... это многое объясняет... Ну, я очень рада, что ты отправляешься в путешествие с нами. Это даст нам прекрасный шанс узнать друг друга получше."

# game/v11/scene3b.rpy:353
translate russian v11_cafe_with_riley_3895912a:

    # charli "I'm looking forward to it! I actually wasn't going to go, but [name] convinced me."
    charli "Я с нетерпением жду этого! Вообще-то я не собирался лететь, но [name] убедил меня."

# game/v11/scene3b.rpy:358
translate russian v11_cafe_with_riley_8ef0b6cc:

    # ri "Well in that case, thank you [name]! *Chuckles*"
    ri "В таком случае, спасибо тебе [name]! *Хихикает*"

# game/v11/scene3b.rpy:363
translate russian v11_cafe_with_riley_41b4a17c:

    # charli "I should finish getting ready for the long flight tomorrow. See you guys!"
    charli "Завтра я должен закончить подготовку к долгому перелету. Увидимся, ребята!"

# game/v11/scene3b.rpy:368
translate russian v11_cafe_with_riley_59b2f8e8:

    # u "Later man."
    u "Давай, мужик"

# game/v11/scene3b.rpy:373
translate russian v11_cafe_with_riley_d3e22b30:

    # u "Someone found a new best friend."
    u "Кто-то нашел нового лучшего друга."

# game/v11/scene3b.rpy:378
translate russian v11_cafe_with_riley_99af0934:

    # ri "Oh don't say it like that, he's sweet. Maybe girls would call you sweet too if you paid them a compliment every once in a while."
    ri "Ох, не говори так, он милый. Может быть, девушки тоже будут называть тебя милым, если ты будешь делать им комплименты время от времени."

# game/v11/scene3b.rpy:383
translate russian v11_cafe_with_riley_b9ae452b:

    # u "I do give compliments!"
    u "Я делаю комплименты!"

# game/v11/scene3b.rpy:388
translate russian v11_cafe_with_riley_02c4444f:

    # ri "Haha, relax! I'm just teasing you... Is someone jealous of the new kid? *Laughs*"
    ri "Хаха, расслабься! Я просто дразню тебя... Кто-то ревнует к новенькому? *Смеётся*"

# game/v11/scene3b.rpy:393
translate russian v11_cafe_with_riley_3639a878:

    # u "I'm not jealous, just surprised to see you hit it off like that."
    u "Я не ревную, просто удивлён, что вы так сдружились."

# game/v11/scene3b.rpy:398
translate russian v11_cafe_with_riley_4231ea00:

    # ri "Maybe I thought he was cute?"
    ri "Может, я считаю, что он симпатичный?"

# game/v11/scene3b.rpy:403
translate russian v11_cafe_with_riley_fcf2d6b5:

    # u "Did you?"
    u "Да?"

# game/v11/scene3b.rpy:408
translate russian v11_cafe_with_riley_093afd15:

    # ri "*Laughs* I'm gonna get on out of here. And now that I think about it, flying is the last thing on my mind, so thank you."
    ri "*Смеётся* Я собираюсь выбраться отсюда. И теперь, когда я думаю об этом, полет - последнее, о чем я думаю, так что спасибо."

# game/v11/scene3b.rpy:413
translate russian v11_cafe_with_riley_70e8d1c9:

    # u "You're welcome. See you around, Riley."
    u "Пожалуйста. Увидимся, Райли"

# game/v11/scene3b.rpy:418
translate russian v11_cafe_with_riley_bde6a20f:

    # ri "See ya."
    ri "Увидимся."

# game/v11/scene3b.rpy:423
translate russian v11_cafe_with_riley_7ee312d1:

    # u "(I'm not jealous, I just... I don't know. )"
    u "(Я не ревную, я просто... Я не знаю.)"

translate russian strings:

    # game/v11/scene3b.rpy:123
    old "Scare her"
    new "Напугать ее"

    # game/v11/scene3b.rpy:276
    old "Not yet"
    new "Ещё нет"

