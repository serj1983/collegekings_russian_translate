# TODO: Translation updated at 2021-11-10 00:46
# Перевёл: serj1983 (post.serj@gmail.com)

# game/v11/scene8.rpy:10
translate russian v11_chicks_house_100e5139:

    # u "I'm thirsty."
    u "Я хочу пить."

# game/v11/scene8.rpy:15
translate russian v11_chicks_house_360684d7:

    # au "Here, give me those bags. Go get something from the kitchen."
    au "Вот, дай мне эти сумки. Иди принеси что-нибудь с кухни."

# game/v11/scene8.rpy:20
translate russian v11_chicks_house_b8911521:

    # u "Alright."
    u "Хорошо."

# game/v11/scene8.rpy:45
translate russian v11_chicks_house_84e004ee:

    # li "Oh be careful, I'm getting ready to clean that up."
    li "Осторожно, я собираюсь это убрать."

# game/v11/scene8.rpy:50
translate russian v11_chicks_house_b898a6eb:

    # u "What happened?"
    u "Что случилось?"

# game/v11/scene8.rpy:55
translate russian v11_chicks_house_ad1c06f1:

    # li "*Sighs* Chloe and Nora got into it last night. They were throwing shit everywhere and now I'm stuck here cleaning it up."
    li "*Вздох* Хлоя и Нора вчера вечером устроили скандал. Они разбрасывали вещи повсюду, и теперь я застряла тут, убирая всё это."

# game/v11/scene8.rpy:60
translate russian v11_chicks_house_93b11488:

    # u "What were they fighting about?"
    u "Из-за чего они ссорились?"

# game/v11/scene8.rpy:65
translate russian v11_chicks_house_b24d78ee:

    # li "I have no idea. They're always fighting about something."
    li "Понятия не имею. Они всегда из-за чего-то ссорятся."

# game/v11/scene8.rpy:70
translate russian v11_chicks_house_3f84d185:

    # u "You're not the first person to bring that up. Plus, I think we all got a taste of it at the charity event."
    u "Ты не первая, кто поднимает этот вопрос. К тому же, я думаю, мы все почувствовали вкус этого на благотворительном мероприятии."

# game/v11/scene8.rpy:75
translate russian v11_chicks_house_da5e0940:

    # li "It's honestly a fucking embarrassment. This isn't what we're supposed to be about."
    li "Честно говоря, это чертовски неловко. Это не то, чем мы должны заниматься."

# game/v11/scene8.rpy:80
translate russian v11_chicks_house_93a9a802:

    # u "Are all the girls feeling the same way?"
    u "Все девушки чувствуют то же самое?"

# game/v11/scene8.rpy:85
translate russian v11_chicks_house_fa0cb85c:

    # li "Yeah, pretty much. They honestly need to calm down."
    li "Да, в значительной степени. Им, честно говоря, нужно успокоиться."

# game/v11/scene8.rpy:90
translate russian v11_chicks_house_9ddcc86d:

    # au "Hey [name], it was nice hanging with you. I would hang out longer, but my sister is getting ready to call and I haven't had a chance to speak with her in a while."
    au "Эй [name], было приятно пообщаться с тобой. Я бы задержалась подольше, но моя сестра должна позвонить, у меня давно не было возможности поговорить с ней."

# game/v11/scene8.rpy:95
translate russian v11_chicks_house_f9691f0d:

    # u "Alright, sounds good."
    u "Ладно."

# game/v11/scene8.rpy:100
translate russian v11_chicks_house_49e9e7d4:

    # u "Sorry, I was hanging with Aubrey earlier."
    u "Извини, я раньше тусовался с Обри."

# game/v11/scene8.rpy:105
translate russian v11_chicks_house_c99875f8:

    # li "No, you're good."
    li "Всё хорошо."

# game/v11/scene8.rpy:110
translate russian v11_chicks_house_31097aaa:

    # u "So what's the plan?"
    u "Так каков план?"

# game/v11/scene8.rpy:115
translate russian v11_chicks_house_825e97f7:

    # li "What do you mean?"
    li "Что ты имеешь в виду?"

# game/v11/scene8.rpy:120
translate russian v11_chicks_house_960078e7:

    # u "What's the plan to get the Chicks back together?"
    u "Каков план по возвращению Чикс вместе?"

# game/v11/scene8.rpy:125
translate russian v11_chicks_house_17910376:

    # li "I don't know, but Chloe needs to do something. She's the president. I understand balancing friendships and running everything can be complicated, but that's part of the job."
    li "Я не знаю, но Хлоя должна что-то сделать. Она президент. Я понимаю, что балансировать между дружбой и управлением может быть сложно, но это её часть работы."

# game/v11/scene8.rpy:130
translate russian v11_chicks_house_e98ae232:

    # u "Have you spoken to her about it?"
    u "Ты говорила с ней об этом?"

# game/v11/scene8.rpy:135
translate russian v11_chicks_house_5e9e480a:

    # li "No, but others have."
    li "Нет, но другие да."

# game/v11/scene8.rpy:140
translate russian v11_chicks_house_b592843e:

    # u "And?"
    u "И?"

# game/v11/scene8.rpy:145
translate russian v11_chicks_house_c6eab9bc:

    # li "And we are where we are now."
    li "И, ты сейчас сам всё видешь"

# game/v11/scene8.rpy:150
translate russian v11_chicks_house_59ca6a00:

    # u "Oh."
    u "Ох."

# game/v11/scene8.rpy:155
translate russian v11_chicks_house_a9135f24:

    # li "I don't know if she's really the best person to be president."
    li "Я не знаю, лучший ли она кандидат на пост президента."

# game/v11/scene8.rpy:160
translate russian v11_chicks_house_b263fce7:

    # u "Who is then?"
    u "Кто же тогда?"

# game/v11/scene8.rpy:165
translate russian v11_chicks_house_a711bad6:

    # li "I don't know."
    li "Я незнаю"

# game/v11/scene8.rpy:170
translate russian v11_chicks_house_a59cbb71:

    # u "What about Aubrey?"
    u "А как насчет Обри?"

# game/v11/scene8.rpy:175
translate russian v11_chicks_house_23222dd2:

    # li "Haha, she's happy where she is. I wish I could be as content as she is. Aubrey's always just happy with life as it is."
    li "Ха-ха, она счастлива на своём месте. Я бы хотела быть такой же довольной, как она. Обри всегда довольна жизнью."

# game/v11/scene8.rpy:176
translate russian v11_chicks_house_c90da53b:

    # li "I wish I was more like that, but I feel like I always need to be doing more."
    li "Я бы хотела быть такой же, но мне кажется, что я всегда должна делать больше."

# game/v11/scene8.rpy:181
translate russian v11_chicks_house_09332657:

    # u "What do you mean?"
    u "Что ты имеешь в виду?"

# game/v11/scene8.rpy:186
translate russian v11_chicks_house_17e3b3ec:

    # li "Is it bad if I'm thinking about running for president?"
    li "Разве это плохо, если я подумываю о том, чтобы баллотироваться в президенты?"

# game/v11/scene8.rpy:187
translate russian v11_chicks_house_87c169a5:

    # li "Like, I'm friends with Chloe and I wouldn't want me running to get between us. There's just so many people that are unhappy with her."
    li "Например, я дружу с Хлоей, и я бы не хотела, чтобы моё решение встало между нами. Есть так много людей, которые недовольны ею."

# game/v11/scene8.rpy:202
translate russian v11_chicks_house_b4f5b0a6:

    # u "I think you'd make a good president. And the only way to find out if the rest of the girls support you is by running and seeing how they vote."
    u "Я думаю, из тебя получился бы хороший президент. И единственный способ узнать, поддерживают ли тебя остальные девочки, - это участвовать в выборах и посмотреть, как они проголосуют."

# game/v11/scene8.rpy:207
translate russian v11_chicks_house_b8fe2c04:

    # li "And if I lose the vote I'll just have a target on my back and divide the sorority even more."
    li "А если я проиграю голосование, у меня будет мишень на спине, и это еще больше разделит сестринство."

# game/v11/scene8.rpy:212
translate russian v11_chicks_house_ffab5778:

    # u "Maybe, but if you think you'd be best then it's only right that you run."
    u "Может быть, но если ты думаешь, что будешь лучшим, то будет правильно."

# game/v11/scene8.rpy:217
translate russian v11_chicks_house_25e7028c:

    # li "That is some good advice."
    li "Это дельный совет."

# game/v11/scene8.rpy:223
translate russian v11_chicks_house_2dc07ab1:

    # u "I don't know what's best."
    u "Я не знаю, как лучше."

# game/v11/scene8.rpy:228
translate russian v11_chicks_house_5e897b84:

    # li "You and I both."
    li "Ты и я."

# game/v11/scene8.rpy:233
translate russian v11_chicks_house_1a79d11e:

    # u "Heard anything about any other girls wanting to run?"
    u "Слышала что-нибудь о других девушках, желающих участвовать в выборах?"

# game/v11/scene8.rpy:238
translate russian v11_chicks_house_c4fe57d5:

    # li "No I haven't. Most of them think we just have to wait until Chloe graduates."
    li "Нет, не знаю. Большинство из них думают, что мы должны подождать, пока Хлоя закончит учёбу."

# game/v11/scene8.rpy:243
translate russian v11_chicks_house_d3543e11:

    # u "Is that so bad?"
    u "Разве это плохо?"

# game/v11/scene8.rpy:248
translate russian v11_chicks_house_8c63bbdd:

    # li "By that time the reputation of the Chicks could be beyond repair. Next semester they'll be preparing for recruiting season and we don't want to do that with all this bad blood."
    li "К тому времени репутация Чикс может оказаться безнадежной. В следующем семестре они будут готовиться к рекрутскому сезону, и мы не хотим этого делать со всей этой дурной кровью."

# game/v11/scene8.rpy:253
translate russian v11_chicks_house_b10872ef:

    # u "Oh yeah, that makes sense."
    u "О да, в этом есть смысл."

# game/v11/scene8.rpy:258
translate russian v11_chicks_house_c3a78f86:

    # li "I better stop talking before one of the girls walks in. I wouldn't want to continue this conversation with one of them right now."
    li "Мне лучше прекратить разговор, пока не вошла одна из девушек. Я бы не хотела продолжать этот разговор с одной из них прямо сейчас."

# game/v11/scene8.rpy:263
translate russian v11_chicks_house_aafa0a58:

    # u "Haha, I need to head home anyway. I'll see you around Linds."
    u "Хаха, мне все равно пора домой. Увидимся с Линдс."

# game/v11/scene8.rpy:268
translate russian v11_chicks_house_efacf4b5:

    # li "Haha, see ya."
    li "Хаха, увидимся."

translate russian strings:

    # game/v11/scene8.rpy:192
    old "You should run"
    new "Тебе следует участвовать"

