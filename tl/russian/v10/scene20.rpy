# TODO: Translation updated at 2021-11-10 00:46

# game/v10/scene20.rpy:41
translate russian v10s20_PhoneContinueW_66785db8:

    # u "(I should reply to Josh.)"
    u "(Я должен ответить Джошу.)"

# game/v10/scene20.rpy:45
translate russian v10s20_PhoneContinueW_8bafdcb6:

    # u "(Okay let's check this out.)"
    u "(Хорошо, сейчас проверим.)"

# game/v10/scene20.rpy:51
translate russian v10s20_PhoneContinueW_eb645489:

    # u "(I'm not surprised Josh is so excited about this. Too bad the girls have to meet him in person too.)"
    u "(Я не удивлён, что Джош так рад этому. Жаль, что девочкам тоже придётся встретиться с ним лично.)"

# game/v10/scene20.rpy:58
translate russian v10s20_PhoneContinueW_9791da08:

    # u "(Hmm, I can see why so many people are talking about it.)"
    u "(Хм, я понимаю, почему так много людей говорят об этом.)"

# game/v10/scene20.rpy:59
translate russian v10s20_PhoneContinueW_a6701ead:

    # u "(It would be kinda weird though if I saw someone I knew.)"
    u "(Было бы странно, если бы я увидел кого-то знакомого.)"

# game/v10/scene20.rpy:60
translate russian v10s20_PhoneContinueW_435bc466:

    # u "(Do you try to match with friends you see on there, or is that weird?)"
    u "(Пытаться знакомиться с друзьями, которых видишь там, или это странно?)"

# game/v10/scene20.rpy:61
translate russian v10s20_PhoneContinueW_7c1c2f0f:

    # u "(I guess I'll figure that out if it ever comes to it. It's about time I go to bed.)"
    u "(Полагаю, я разберусь с этим, если когда-нибудь до этого дойдёт Мне пора спать.)"

# game/v10/scene20.rpy:98
translate russian v10s20_PhoneContinue2_66785db8:

    # u "(I should reply to Josh.)"
    u "(Я должен ответить Джошу.)"

# game/v10/scene20.rpy:102
translate russian v10s20_PhoneContinue2_8bafdcb6:

    # u "(Okay let's check this out.)"
    u "(Хорошо, сейчас проверим.)"

# game/v10/scene20.rpy:108
translate russian v10s20_PhoneContinue2_eb645489:

    # u "(I'm not surprised Josh is so excited about this. Too bad the girls have to meet him in person too.)"
    u "(Я не удивлён, что Джош так рад этому. Жаль, что девочкам тоже придётся встретиться с ним лично.)"

# game/v10/scene20.rpy:115
translate russian v10s20_PhoneContinue2_9791da08:

    # u "(Hmm, I can see why so many people are talking about it.)"
    u "(Хм, я понимаю, почему так много людей говорят об этом.)"

# game/v10/scene20.rpy:116
translate russian v10s20_PhoneContinue2_a6701ead:

    # u "(It would be kinda weird though if I saw someone I knew.)"
    u "(Было бы странно, если бы я увидел кого-то знакомого.)"

# game/v10/scene20.rpy:117
translate russian v10s20_PhoneContinue2_435bc466:

    # u "(Do you try to match with friends you see on there, or is that weird?)"
    u "(Пытаться знакомиться с друзьями, которых видишь там, или это странно?)"

# game/v10/scene20.rpy:118
translate russian v10s20_PhoneContinue2_7c1c2f0f:

    # u "(I guess I'll figure that out if it ever comes to it. It's about time I go to bed.)"
    u "(Полагаю, я разберусь с этим, если когда-нибудь до этого дойдёт Мне пора спать.)"

translate russian strings:

    old "Then why are you acting clueless? Any good matches?"
    new "Тогда почему ты ведешь себя невежественно? Есть какие-нибудь хорошие совпадения?"
    old "I haven't used it yet."
    new "Я им еще не пользовался."
    old "WHAT! I've been talking to so many hot chicks. What are you waiting for?"
    new "ЧТО! Я общался со столькими горячими цыпочками. Чего ты ждешь?"
    old "Hop on it."
    new "Запрыгивай."
    old "You been under a rock? Get it and check it out. You probably won't get as many girls as me, but tell me how it goes."
    new "Ты был под камнем? Достань и проверь. Возможно, у тебя не будет столько девушек, как у меня, но потом расскажешь, как всё прошло."
    old "Haha, okay."
    new "Хаха, хорошо."
    old "So what do think!? Any good ones for you?"
    new "И что ты думаешь? Есть хорошенькие для тебя?"
    old "What are you talking about?"
    new "О чем ты говоришь?"
    old "Wait, you don't know what I'm talking about? Everybody on campus knows about it. Except for you I guess."
    new "Подожди, ты не знаешь о чём я говорю? Все в кампусе знают об этом. Кроме тебя, наверное."
    old "What does everyone know about?"
    new "О чем все знают?"
    old "The new dating app Simplr, they literally just launched."
    new "Новое приложение для знакомств Simplr, буквально только что запущенное."
    old "You really haven't heard about it?"
    new "Ты действительно не слышал о нём?"
    old "Oh yeah, I heard."
    new "О да, я слышал."
    old "I have no idea what you're talking about."
    new "Я понятия не имею, о чем ты говоришь."
# TODO: Translation updated at 2023-06-21 15:57

# game/v10/scene20.rpy:42
translate russian v10_room_mon_night_66785db8:

    # u "(I should reply to Josh.)"
    u "(Я должен ответить Джошу.)"

# game/v10/scene20.rpy:45
translate russian v10_room_mon_night_8bafdcb6:

    # u "(Okay let's check this out.)"
    u "(Хорошо, сейчас проверим.)"

# game/v10/scene20.rpy:51
translate russian v10_room_mon_night_eb645489:

    # u "(I'm not surprised Josh is so excited about this. Too bad the girls have to meet him in person too.)"
    u "(Я не удивлён, что Джош так рад этому. Жаль, что девочкам тоже придётся встретиться с ним лично.)"

# game/v10/scene20.rpy:56
translate russian v10_room_mon_night_9791da08:

    # u "(Hmm, I can see why so many people are talking about it.)"
    u "(Хм, я понимаю, почему так много людей говорят об этом.)"

# game/v10/scene20.rpy:57
translate russian v10_room_mon_night_a6701ead:

    # u "(It would be kinda weird though if I saw someone I knew.)"
    u "(Было бы странно, если бы я увидел кого-то знакомого.)"

# game/v10/scene20.rpy:58
translate russian v10_room_mon_night_435bc466:

    # u "(Do you try to match with friends you see on there, or is that weird?)"
    u "(Пытаться знакомиться с друзьями, которых видишь там, или это странно?)"

# game/v10/scene20.rpy:59
translate russian v10_room_mon_night_7c1c2f0f:

    # u "(I guess I'll figure that out if it ever comes to it. It's about time I go to bed.)"
    u "(Полагаю, я разберусь с этим, если когда-нибудь до этого дойдёт Мне пора спать.)"

# game/v10/scene20.rpy:108
translate russian v10_room_mon_night_66785db8_1:

    # u "(I should reply to Josh.)"
    u "(Я должен ответить Джошу.)"

# game/v10/scene20.rpy:111
translate russian v10_room_mon_night_8bafdcb6_1:

    # u "(Okay let's check this out.)"
    u "(Хорошо, сейчас проверим.)"

# game/v10/scene20.rpy:117
translate russian v10_room_mon_night_eb645489_1:

    # u "(I'm not surprised Josh is so excited about this. Too bad the girls have to meet him in person too.)"
    u "(Я не удивлён, что Джош так рад этому. Жаль, что девочкам тоже придётся встретиться с ним лично.)"

# game/v10/scene20.rpy:122
translate russian v10_room_mon_night_9791da08_1:

    # u "(Hmm, I can see why so many people are talking about it.)"
    u "(Хм, я понимаю, почему так много людей говорят об этом.)"

# game/v10/scene20.rpy:123
translate russian v10_room_mon_night_a6701ead_1:

    # u "(It would be kinda weird though if I saw someone I knew.)"
    u "(Было бы странно, если бы я увидел кого-то знакомого.)"

# game/v10/scene20.rpy:124
translate russian v10_room_mon_night_435bc466_1:

    # u "(Do you try to match with friends you see on there, or is that weird?)"
    u "(Пытаться знакомиться с друзьями, которых видишь там, или это странно?)"

# game/v10/scene20.rpy:125
translate russian v10_room_mon_night_7c1c2f0f_1:

    # u "(I guess I'll figure that out if it ever comes to it. It's about time I go to bed.)"
    u "(Полагаю, я разберусь с этим, если когда-нибудь до этого дойдёт Мне пора спать.)"

