# TODO: Translation updated at 2021-11-10 00:46

# game/v9/scene17.rpy:11
translate russian v9_room_fri_morn_eef583a3:

    # ch "Wake up [name], Wolves aren't nocturnal. Go be productive. And someone sent you some mail Mr. Popular."
    ch "Просыпайся [name], Волки не водят ночной образ жизни. Иди, будь продуктивным. И кто-то прислал тебе письмо, мистер Популярный."

# game/v9/scene17.rpy:16
translate russian v9_room_fri_morn_8da0ae81:

    # u "Early bird gets the worm and yada yada yada."
    u "Ранняя пташка получает червячка и ля-ля-ля."

# game/v9/scene17.rpy:21
translate russian v9_room_fri_morn_4471b800:

    # ch "Right!"
    ch "Верно!"

# game/v9/scene17.rpy:35
translate russian v9_room_fri_morn_db7a4e2e:

    # u "(Over the semester you've been way more than a student, a true confidant. I hope I'll have the opportunity to return the favor. With gratitude, Miss Rose.)"
    u "(В течение семестра ты был не просто студентом, а настоящим доверенным лицом. Надеюсь, у меня будет возможность вернуть тебе должок. С благодарностью, мисс Роуз.)"

# game/v9/scene17.rpy:40
translate russian v9_room_fri_morn_f92a2101:

    # u "(Ms. Rose, did she get a divorce? And what does she mean return the favor? Guess I'll find out soon.)"
    u "(Мисс Роуз, она развелась? И что она имеет в виду под долгом? Думаю, я скоро узнаю.)"

# game/v9/scene17.rpy:55
translate russian s17_PhoneContinueW_0f06666e:

    # "(I should text Lauren.)"
    "(Я должен написать Лорен.)"

# game/v9/scene17.rpy:58
translate russian s17_PhoneContinueW_23ba1da1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Мне лучше собраться и отправиться туда. Надеюсь, там не слишком много работы.)"

# game/v9/scene17.rpy:66
translate russian s17_PhoneContinueW_5cdddfcb:

    # gr "Wake up, [name], Apes aren't lazy assholes. Get up. Go be productive. Oh, and I'd avoid the toilet. Cameron backed it up again."
    gr "Подъём, [name], Приматы не ленивые засранцы. Вставай. Иди и будь продуктивным. О, и я бы избегал туалета. Кэмерон снова занял его."

# game/v9/scene17.rpy:71
translate russian s17_PhoneContinueW_19541c97:

    # u "*Drowsy* Uhh, yeah ok. I'm up, and ewww."
    u "*Сонный* Да, хорошо. Я встал, и ууу."

# game/v9/scene17.rpy:76
translate russian s17_PhoneContinueW_68aafa48:

    # gr "Good."
    gr "Хорошо."

# game/v9/scene17.rpy:81
translate russian s17_PhoneContinueW_347d66bc:

    # u "(Is this guy my fucking dad? Jesus.)"
    u "(Этот парень - мой грёбаный отец? Господи.)"

# game/v9/scene17.rpy:96
translate russian s17_PhoneContinueA_0f06666e:

    # "(I should text Lauren.)"
    "(Я должен написать Лорен.)"

# game/v9/scene17.rpy:99
translate russian s17_PhoneContinueA_23ba1da1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Мне лучше собраться и отправиться туда. Надеюсь, там не слишком много работы.)"

translate russian strings:

    # game/v9/scene17.rpy:47
    old "Hey, would yould you like to come over to my dorm and help me? I need help with my Deer initiation. They want me to help with the annual charity day event they have."
    new "Эй, не хочешь ли ты прийти ко мне в общежитие и помочь мне? Мне нужна помощь с посвящением в Лани. Они хотят, чтобы я помогла с ежегодным благотворительным мероприятием, которое они проводят."

    # game/v9/scene17.rpy:48
    old "Sure, OMW!"
    new "Конечно, ммм!"

    # game/v9/scene17.rpy:49
    old "Ok :) See you soon!"
    new "Ok :) Увидимся тогда!"

# TODO: Translation updated at 2023-06-21 15:57

# game/v9/scene17.rpy:54
translate russian v9_room_fri_morn_8e7cf5c5:

    # u "(I should text Lauren.)"
    u "(Я должен написать Лорен.)"

# game/v9/scene17.rpy:56
translate russian v9_room_fri_morn_23ba1da1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Мне лучше собраться и отправиться туда. Надеюсь, там не слишком много работы.)"

# game/v9/scene17.rpy:64
translate russian v9_room_fri_morn_5cdddfcb:

    # gr "Wake up, [name], Apes aren't lazy assholes. Get up. Go be productive. Oh, and I'd avoid the toilet. Cameron backed it up again."
    gr "Подъём, [name], Приматы не ленивые засранцы. Вставай. Иди и будь продуктивным. О, и я бы избегал туалета. Кэмерон снова занял его."

# game/v9/scene17.rpy:69
translate russian v9_room_fri_morn_19541c97:

    # u "*Drowsy* Uhh, yeah ok. I'm up, and ewww."
    u "*Сонный* Да, хорошо. Я встаю, и ууу."

# game/v9/scene17.rpy:74
translate russian v9_room_fri_morn_68aafa48:

    # gr "Good."
    gr "Хорошо."

# game/v9/scene17.rpy:79
translate russian v9_room_fri_morn_347d66bc:

    # u "(Is this guy my fucking dad? Jesus.)"
    u "(Этот парень - мой грёбаный отец? Господи.)"

# game/v9/scene17.rpy:93
translate russian v9_room_fri_morn_8e7cf5c5_1:

    # u "(I should text Lauren.)"
    u "(Я должен написать Лорен.)"

# game/v9/scene17.rpy:95
translate russian v9_room_fri_morn_23ba1da1_1:

    # u "(I better get ready and head over there. Hope it's not too much work.)"
    u "(Мне лучше собраться и отправиться туда. Надеюсь, там не слишком много работы.)"

