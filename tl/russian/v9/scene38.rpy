# TODO: Translation updated at 2021-11-10 00:46

# game/v9/scene38.rpy:21
translate russian v9_walk_li_txt_1d8a7ae7:

    # u "(Ugh, this is such a mess. How did I get stuck in the middle of those two?)"
    u "(Фу, какая неразбериха. Как я застрял между этими двумя?)"

# game/v9/scene38.rpy:23
translate russian v9_walk_li_txt_ed398106:

    # u "(I may need some help if they run into each other again)"
    u "(Мне может понадобиться помощь, если они снова столкнутся друг с другом.)"

# game/v9/scene38.rpy:32
translate russian v9_walk_li_txt_112e72aa:

    # u "(Please no more drama)"
    u "(Пожалуйста, не надо больше драмы)"

# game/v9/scene38.rpy:46
translate russian s38_PhoneContinue_73e9c274:

    # "(I should reply to Lindsey.)"
    "(Я должен ответить Линдси.)"

# game/v9/scene38.rpy:53
translate russian s38_PhoneContinue_744c4169:

    # u "(I suppose I should head to see Lindsey.)"
    u "(Полагаю, мне стоит отправиться к Линдси.)"

# game/v9/scene38.rpy:60
translate russian s38_PhoneContinue_538b62a3:

    # u "(Shit.)"
    u "(Блин.)"

translate russian strings:

    # game/v9/scene38.rpy:8
    old "Great. See ya soon"
    new "Отлично. До скорого."

    # game/v9/scene38.rpy:12
    old ":("
    new ":("

    # game/v9/scene38.rpy:13
    old "Please don't hate me. You know I want to. I really, really want to."
    new "Пожалуйста, не ненавидь меня. Ты знаешь, что я хочу. Я очень, очень хочу."

    # game/v9/scene38.rpy:14
    old "No hard feelings. Bye"
    new "Никаких обид. Пока"

    # game/v9/scene38.rpy:15
    old "Maybe after the fight?"
    new "Может быть, после боя?"

    # game/v9/scene38.rpy:34
    old "Hey Freshmeat! How's it hangin?"
    new "Привет, Freshmeat! Как дела?"

    # game/v9/scene38.rpy:35
    old "Hey Linds ;)"
    new "Привет, Линдс ;)"

    # game/v9/scene38.rpy:36
    old "I've decided to allow the nickname under one condition"
    new "Я разрешаю этот ник при одном условии."

    # game/v9/scene38.rpy:37
    old "Really? And what's that?"
    new "Правда? И что это?"

    # game/v9/scene38.rpy:38
    old "Come hang out with me"
    new "Погуляй со мной"

    # game/v9/scene38.rpy:39
    old "Hell yeah! Be right there, Linds!"
    new "Да, конечно! Сейчас буду, Линдс!"

    # game/v9/scene38.rpy:40
    old "Aww, I wish I could, but I gotta get ready for the brawl"
    new "Оу, я бы хотел, но мне нужно готовиться к поединкам."

# TODO: Translation updated at 2023-06-21 15:57

# game/v9/scene38.rpy:47
translate russian v9_walk_li_txt_f9e9813d:

    # u "(I should reply to Lindsey.)"
    u "(Я должен ответить Линдси.)"

# game/v9/scene38.rpy:53
translate russian v9_walk_li_txt_744c4169:

    # u "(I suppose I should head to see Lindsey.)"
    u "(Полагаю, мне стоит отправиться к Линдси.)"

# game/v9/scene38.rpy:60
translate russian v9_walk_li_txt_538b62a3:

    # u "(Shit.)"
    u "(Дерьмо.)"

